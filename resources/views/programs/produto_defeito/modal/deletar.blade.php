...
@section('content')
<form name="form_delete_produto_defeito" id="form_delete_produto_defeito">
    @csrf
    <div class="row">
        <div class="col-sm">
    ...
    <div class="content-buttons float-right mt-3">
        ...
        {{ Form::button('Excluir', ['id' => 'form_delete_produto_defeito_btn', 'class' => 'btn btn-danger']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_delete_produto_defeito_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('produto_defeito.excluir') }}",
                dataType: 'json',
                method: 'POST',
                data: $(document).find('#form_delete_produto_defeito').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_delete_produto_defeito').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        showErrorsInputs('#form_delete_produto_defeito', field, errors[field]);
                    }
                }
            })
        });
