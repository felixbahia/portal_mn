...
@section('content')
<form id="form_add_produto_defeito">
    @csrf
    <div class="form-row">
        <div class="col-sm">
    ...
    <div class="form-row float-right mt-3">
        {{ Form::button('Cadastrar', ['id' => 'form_add_produto_defeito_btn', 'class' => 'btn btn-success']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_add_produto_defeito_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('produto_defeito.salvar') }}",
                dataType: 'json',
                method: 'POST', 
                data: $(document).find('#form_add_produto_defeito').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_add_produto_defeito').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        ...
                        showErrorsInputsAdd('#form_add_produto_defeito', field, errors[field]);
                    }
                }
            })
        });
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $("#form_add_produto_defeito");
