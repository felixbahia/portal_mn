...
<div class="content-table">
    <table class="table table-striped" id="table-filters-video">
        <thead>
            <tr>
                <th>Video</th>
                <th class="descricao">Descrição</th>
                <th class="descricao">Perfil</th>
                 <th>Editar</th>
                <th>Excluir</th>
            </tr>
        </thead>
...
@section('script-footer')
    $(document).ready(function(){
        ...
        table_filters_video = $('#table-filters-video')
        ...
        table_filters_video.on('draw', function () {
            ...
            $(document).find(".bt-view").on("click", function(){
                modalVideo($(this).data('id'));
            })
        });
    });

    function filtro(){
        table_filters_video.clear().draw();
        $.ajax({
            url: "{{ route('video.filtro') }}", 
            dataType: 'json',
            data: $(document).find('#form_video').serialize(),
            method: 'POST',
            success: function(callback){
                ...
                if(dados.length > 0){
                    ...
                    table_filters_video.rows.add(fields_filter).draw().nodes();
                }
            },
            error: function(callback){
                message('Atenção', 'Nenhum Video localizado.');
            }
        });
    }
    ...
    function modalAdicionar() {
        $.ajax({
            url: '{{ route("video.modal.salvar") }}',
            data: {_token: '{{ csrf_token() }}'},
            method: 'POST',
            success: function(body){
                var title = 'Cadastro de {{ CustomView::programaName() }}';
                createModal("modal_add_video", title, body, '');
            },
            error: function(data){
    ...
    function modalEditar($id) {
        $.ajax({
            url: '{{ route("video.modal.editar") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Editar {{ CustomView::programaName() }}';
                createModal("modal_edit_video", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
    ...
    function modalDeletar($id) {
        $.ajax({
            url: '{{ route("video.modal.excluir") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Deletar {{ CustomView::programaName() }}';
                createModal("modal_delete_video", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
    ...
    function modalVideo($id) {
        ...
        $.ajax({
            url: '{{ route("video.modal.video") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){

                var title = 'Video {{ CustomView::programaName() }}';
