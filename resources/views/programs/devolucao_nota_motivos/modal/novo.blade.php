...
@section('content')
<form action="#" id='form-novo-motivo' onsubmit="return false;">
    @csrf
    <div class="row">
        <div class="form-group col-sm-12">
            {!! Form::label('descricao_modal', 'Descrição', ['class'=>'input-label']) !!}
            {!! Form::text('descricao', '', ['id' => 'descricao_modal', 'class' => 'form-control']) !!}
        </div>
        ...
        <div class="form-group col-md-12">
            <div class="form-group  form-check form-check-inline">
                ...
                {{ Form::radio('afeta_premiacao', 'sim', '', ['id' => 'afeta_premiacao', 'class' => 'form-check-input']) }}
            </div>
            <div class="form-group  form-check form-check-inline">
                {!! Form::label('afeta_premiacao', 'Não', ['class'=>'form-check-label']) !!}
                {!! Form::radio('afeta_premiacao', 'nao', '', ['id' => 'afeta_premiacao', 'class' => 'form-check-input']) !!}
            </div>
        </div>
        <div class="form-check col-sm-12">
        ...
        <div class="form-group col-md-12">
            <div class="form-group  form-check form-check-inline">
                ...
                {{ Form::radio('assinatura_pedido', 'sim', '', ['id' => 'assinatura_pedido', 'class' => 'form-check-input']) }}
            </div>
            <div class="form-group  form-check form-check-inline">
                {!! Form::label('assinatura_pedido', 'Não', ['class'=>'form-check-label']) !!}
                {!! Form::radio('assinatura_pedido', 'nao', '', ['id' => 'assinatura_pedido', 'class' => 'form-check-input']) !!}
            </div>
        </div>
    </div>
    <div class="row">
...
<script>
    ...
    function salvarNovo(){
        ...
        var form = $(document).find('#form-novo-motivo');

        form.find('.error-message').remove();
        form.find('.error-input').removeClass('error-input');

        $.ajax({
            url: '{{ route("devolucao_nota_motivo.salvar.novo") }}',
            dataType: 'json',
            method: 'POST',
            data: form.serialize(),
            success: function(data){
                message('Atenção', 'Motivo salvo com sucesso')
                $(document).find('#modal-novo-motivo').modal('hide');
                buscarMotivos();
            },
