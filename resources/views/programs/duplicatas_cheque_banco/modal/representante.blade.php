    ...
    <div class="content-table">
        <table class="table table-striped table-filter-dialog footer-pequeno" id="table-clientes-atrasos">
            ...
            <tbody>
                @foreach ($retorno as $dado)
                    <tr>
                        ...
                        <td>
                            <a href="#" class="modal-titulos-aberto-representante" data-route="{{ route('duplicatas_cheque_banco.modal.titulo.representantes') }}" data-abertura='aberto' data-total='{{ $saidatotal }}' data-abertura-geral='{{ $aberturageral }}' data-representante='true' data-title-modal="{{ ($filter['total'] != 'true') ? $dado['banco'] : 'Todos' }} - TITULOS FATURADOS EM ABERTO -  {{  $dado['representante']   }}" data-titulo_id="{{ $dado['titulo_id'] }}" data-filter="{{ $dado['filter'] }}" >{{ $dado['aberto'] }}</a>
                        </td>
                        <td>
                            <a href="#" class="modal-titulos-vencido-representante" data-route="{{ route('duplicatas_cheque_banco.modal.titulo.representantes') }}" data-abertura='vencido' data-total='{{ $saidatotal }}' data-abertura-geral='{{ $aberturageral }}' data-representante='true' data-title-modal="{{  ($filter['total'] != 'true') ? $dado['banco'] : 'Todos' }} - TITULOS FATURADOS VENCIDOS -  {{  $dado['representante']   }}" data-titulo_id="{{ $dado['titulo_id'] }}" data-filter="{{ $dado['filter'] }}" >{{ $dado['vencido'] }}</a>
                        </td>
                        <td>
                            <a href="#" class="modal-titulos-total-representante" data-route="{{ route('duplicatas_cheque_banco.modal.titulo.representantes') }}" data-abertura='total' data-total='{{ $saidatotal }}' data-abertura-geral='{{ $aberturageral }}' data-representante='true' data-title-modal="{{  ($filter['total'] != 'true') ? $dado['banco'] : 'Todos' }} - TITULOS FATURADOS TOTAL -  {{  $dado['representante']   }}" data-titulo_id="{{ $dado['titulo_id'] }}" data-filter="{{ $dado['filter'] }}" >{{ $dado['total'] }}</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
                <td>Total</td>
                <td>
                    <a href="#" class="modal-titulos-aberto-representante-total" data-route="{{ route('duplicatas_cheque_banco.modal.titulo.representantes') }}" data-abertura='aberto' data-abertura-geral='{{ $aberturageral }}' data-total='{{ $saidatotal }}' data-representante='false' data-title-modal="TODOS - TITULOS FATURADOS EM ABERTO - TODOS" data-titulo_id="" data-filter="{{ $total['filter'] }}">{{ $total['aberto'] }}</a>
                </td>
                <td>
                    <a href="#" class="modal-titulos-vencido-representante-total" data-route="{{ route('duplicatas_cheque_banco.modal.titulo.representantes') }}" data-abertura='vencido' data-abertura-geral='{{ $aberturageral }}' data-total='{{ $saidatotal }}' data-representante='false' data-title-modal="TODOS - TITULOS FATURADOS VENCIDOS - TODOS" data-titulo_id="" data-filter="{{ $total['filter'] }}">{{ $total['vencido'] }}</a>
                </td>
                <td>
                    <a href="#" class="modal-titulos-geral-representante-total" data-route="{{ route('duplicatas_cheque_banco.modal.titulo.representantes') }}" data-abertura='total' data-abertura-geral='{{ $aberturageral }}' data-total='{{ $saidatotal }}' data-representante='false' data-title-modal="TODOS - TITULOS FATURADOS TOTAL - TODOS" data-titulo_id="" data-filter="{{ $total['filter'] }}">{{ $total['total'] }}</a>
                </td>
            </tfoot>  
        </table>
    </div>
...
function showModalOpenTitulos($this){
        ...
        $.ajax({
            url: $url,
            method: 'POST',
            data: {_token: "{{ csrf_token() }}", filters: $filter, titulo_id : $titulo_id, abertura : $abertura, aberturageral: $aberturageral, total : $total , representante : $representante},
