...
function createBtViewClienteAtraso($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.cliente') }}\" data-filter=\""+$this.filter+"\" data-abertura='aberto' data-cliente='true' data-abertura-geral='false' data-total='false' data-title='TITULOS FATURADOS - CLIENTE - Representante - "+$this.banco+"' class='bt-view bt-clinete-atraso'></a>"
    return html;
}

function createBtViewRepresentanteAtraso($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.representantes') }}\" data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - Representante - "+$this.banco+"' data-abertura='total' data-abertura-geral='false' data-total='false' data-representante='true'  class='bt-view bt-representante-atraso'></a>"
    return html;
}

function createBtViewAbertoAtraso($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.titulo.titulos_abertura') }}\" data-abertura='aberto' data-total='false' data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - ABERTO - "+$this.banco+"' class='bt-titulos-abertos'>"+$this.aberto+"</a>"
    return html;
}

function createBtViewVencidoAtraso($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.titulo.titulos_abertura') }}\" data-abertura='vencido' data-total='false' data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - VENCIDO -"+$this.banco+"' class='bt-titulos-vencidos'>"+$this.vencido+"</a>"
    return html;
}

function createBtViewTotal($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.titulo.titulos_abertura') }}\" data-abertura='total' data-total='false' data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - TOTAL - "+$this.banco+"' class='bt-titulos-total'>"+$this.total+"</a>"
    return html;
}

function createBtViewTotalAberto($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.titulo.titulos_abertura') }}\" data-abertura='aberto' data-total='true' data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - ABERTO - TODOS' class='bt-titulos-total-aberto'>"+$this.aberto+"</a>"
    return html;
}

function createBtViewTotalVencido($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.titulo.titulos_abertura') }}\" data-abertura='vencido' data-total='true' data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - VENCIDO - TODOS' class='bt-titulos-total-vencido'>"+$this.vencido+"</a>"
    return html;
}

function createBtViewTotalAberturas($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.titulo.titulos_abertura') }}\" data-abertura='total' data-total='true' data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS - TOTAL - TODOS' class='bt-titulos-total-geral'>"+$this.total+"</a>"
    return html;
}

function createBtViewTotalClientes($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.cliente') }}\" data-abertura='total' data-abertura-geral='true' data-cliente='false' data-busca='' data-total='true' data-filter=\""+$this.filter+"\"  data-title='TITULOS FATURADOS POR CLIENTE - TOTAL - TODOS' class='bt-view bt-titulos-total-clientes'></a>"
    return html;
}

function createBtViewTotalRepresentantes($this){
    var html = "<a href=\"#\" data-route=\"{{ route('duplicatas_cheque_banco.modal.representantes') }}\" data-filter=\""+$this.filter+"\" data-title='TITULOS FATURADOS POR Representante - TOTAL - TODOS' data-abertura-geral='true' data-abertura='total' data-total='true' data-representante='false' data-cliente='null' class='bt-view bt-titulos-total-representantes'></a>"
    return html;
}
...
function filter(){
    ...
    var i=0;
    var data_filtro =  $(document).find("#form_filter_atrasos_equipe").find('#data_filtro').val(); 
    $(table_filters_atrasos.column(i++).footer()).html('');
    $(table_filters_atrasos.column(i++).footer()).html('');
    $(table_filters_atrasos.column(i++).footer()).html('');
    ...
    $(table_filters_atrasos.column(i++).footer()).html('');
    $.ajax({
        url: "{{ route('duplicatas_cheque_banco.filter') }}",
        data: $data_form,
        method: 'POST',
        success: function(callback){
            if(callback.status == 'sucess'){
                ...
                if(linhas){
                  ...
                  var y=0;
                    ...
                    if(data_filtro == 'emissao'){
                        $(table_filters_atrasos.column(3).visible( true ));
                         $(table_filters_atrasos.column(3).footer()).html((callback.response.total.pago) ? callback.response.total.pago : '');
                        
                    }else{
                        $(table_filters_atrasos.column(3).visible( false ));
                    }
                    $(table_filters_atrasos.column(4).footer()).html((callback.response.total.total) ? createBtViewTotalAberturas(callback.response.total) : '');
                    $(table_filters_atrasos.column(5).footer()).html((callback.response.total.total) ? createBtViewTotalClientes(callback.response.total) : '');
                    $(table_filters_atrasos.column(6).footer()).html((callback.response.total.total) ? createBtViewTotalRepresentantes(callback.response.total) : '');
...
function showModalClienteAtraso($this){
    ...
    xhr = $.ajax({
        url: url,
        data: {_token: "{{ csrf_token() }}", filters : filter, abertura : abertura, total : total, busca : $busca, aberturageral : $aberturageral,data_filtro : data_filtro},
        method: 'POST',
        success: function(body){
...
function showModalRepresentanteAtraso($this){
    ...
    xhr = $.ajax({
        url: url,
        data: {_token: "{{ csrf_token() }}", filters : filter, abertura : $abertura, aberturageral: $aberturageral, total : $total , representante : $representante,data_filtro : data_filtro},
        method: 'POST',
        success: function(body){
...
function showModalAberturas($this){
   ...
   var data_filtro =  $(document).find("#form_filter_atrasos_equipe").find('#data_filtro').val(); 
    ...
    xhr = $.ajax({
        url: url,
        data: {_token: "{{ csrf_token() }}", filters : filter, abertura : abertura, total : total,data_filtro : data_filtro},
        method: 'POST',
        success: function(body){
...
function showModalOpenTitulosRepresentantesTotal($this){
    ...
    $.ajax({
        url: $url,
        method: 'POST',
        data: {_token: "{{ csrf_token() }}", filters: $filter, clientes : $clientes, abertura : $abertura, aberturageral: $aberturageral, total : $total , representante : $representante,data_filtro : data_filtro},
        success: function(body){
