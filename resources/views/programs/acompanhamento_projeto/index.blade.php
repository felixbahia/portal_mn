...
function filterAjax(){
    ...
    $.ajax({
        url: '{{ route('projeto.acompanhamento.filtro')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            for (var fields in data.response.dados){
                temp_array = [
                    ...
                    createDialog(data.response.dados[fields].prazo, 'prazo', data.response.filtro, data.response.dados[fields].id,"{{ route('projeto.acompanhamento.modal.dialog') }}", data.response.dados[fields].descricao, data.response.dados[fields].natureza),
                    createDialog(data.response.dados[fields].atraso, 'atraso', data.response.filtro, data.response.dados[fields].id,"{{ route('projeto.acompanhamento.modal.dialog') }}", data.response.dados[fields].descricao, data.response.dados[fields].natureza),
                    createDialog(data.response.dados[fields].total, 'total', data.response.filtro, data.response.dados[fields].id,"{{ route('projeto.acompanhamento.modal.dialog') }}", data.response.dados[fields].descricao, data.response.dados[fields].natureza),
                ];
...
function createDialog($value, $tipo, $filtro, $id, $url, $descricao, $natureza){

    html = "<a href=\"#\" data-url=\""+$url+"\" data-id=\""+$id+"\" data-title=\""+$descricao+" - "+$tipo+"\" data-tipo=\""+$tipo+"\" data-filtro=\""+$filtro+"\" data-natureza=\""+$natureza+"\" onclick=\"dialog($(this));\">"+$value+"</a>"
