@extends('layouts.page-dialog')

@section('content')
<form action="#" name='form-novo-status' id='form-novo-status' onsubmit="return false;">
    @csrf
    {!! Form::hidden('id', $id) !!}
    <div class="row">
        <div class="col">
            {!! Form::label('descricao_modal', 'Status') !!}
            {!! Form::text('descricao', $descricao, ['id' => 'descricao_modal', 'class' => 'form form-control']) !!}
        </div>
    </div>
    <div class="row">
        <div class="col text-right">
            {!! Form::checkbox('selecionavel', true, $selecionavel, ['id' => 'selecionavel_modal' ]) !!}
            {!! Form::label('selecionavel_modal', 'Selecionável como etapa') !!}
        </div>
    </div>
    <div class="row mt-3">
        <div class="col">
            {!! Form::button('Salvar', ['id' => 'btn-salvar-novo-modal', 'class' => 'btn btn-success float-right']) !!}
        </div>
    </div>
</form>

<script>
    $(document).ready(function(){
        $(document).find('#btn-salvar-novo-modal').on('click', function(){
            salvarEdicaoStatus();
        });

        $(document).find('#status_modal').on('keypress', function(e){
            if(e.which == 13) {
                salvarEdicaoStatus();
            }
        });

    });

    function salvarEdicaoStatus(){
        form = $(document).find('#form-novo-status');

        $.ajax({
            url: '{{ route("devolucao_nota_status.salvar.editar") }}',
            method: 'POST',
            data: form.serialize(),
            success: function(data){
                $(document).find('#modal-editar-status').modal('hide');
                buscarStatus();
            },
            error: function callback(data){
                message('Atenção!', data.responseJSON.message)  
            }
        });
    }
</script>
@endsection