...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        ...
        <div class="form-group col-lg-1">
            <div>
                {{ Form::select('estados', $estados, 2, ['class' => 'form-control', 'placeholder' => 'Todos'])}}
            </div>
        </div>
    </div>
...
function showModalCreateProjeto(){
    $.ajax({
        ...
        success: function(body){
            ...
            createModal('modal_lancamento_projeto_adicionar', title, body, 'modal-lg');
        }
    });
}
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('lancamento_projeto.revisao.filter')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            produtos = [];
            for (var fields in data.response){
                if(data.response[fields].status_codigo == 1 || data.response[fields].status_codigo == 2 || data.response[fields].status_codigo == 3){
                    if(data.response[fields].status_codigo == 3){
                        temp_array = [
                            '',
                            ...
                            trueOuFalse(data.response[fields].aprovar),
                            '',
                            createBtnEdit("{{ route('lancamento_projeto.revisao.modal.editar') }}", data.response[fields].id, data.response[fields].mostrar_botao_aprovar),
                            '',
                        ];
                    }else{
                        temp_array = [
                            createBtReprove(data.response[fields].id),
                            data.response[fields].num_projeto,
                            ajusteTamanhoTable(data.response[fields].nome_projeto),
                            ajusteTamanhoTable(data.response[fields].cliente),
                            data.response[fields].data,
                            ...
                            data.response[fields].status,
                            ajusteTamanhoTable(data.response[fields].vendedor),
                            trueOuFalse(data.response[fields].faccao_aprovar),
                            '',
                            createBtnEdit("{{ route('lancamento_projeto.revisao.modal.editar') }}", data.response[fields].id, data.response[fields].mostrar_botao_aprovar),
                            createBtAprove(data.response[fields].aprovar, data.response[fields].id),
                        ];
                    }
...
function createBtnEdit($url, $id, $mostrar_botao_aprovar){
    var $html = "<a href=\"#\" data-mostrar=\""+$mostrar_botao_aprovar+"\" data-route=\""+$url+"\" data-id=\""+$id+"\" data-modal=\"modal-lg\" data-title_modal=\"Revisão {{ CustomView::programaName() }}\" class=\"bt-edit\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Revisão\"></a>";
    return $html;
}
...
function createBtDuplicar($this){
    var html = '';
...
function showModal($this){
    ...
    var $id = $($this).data("id");
    var $mostrar_botao_aprovar = $($this).data("mostrar");
    var modal_class = $($this).data("modal");
    var title = $($this).data("title_modal");
    $.ajax({
        url: url,
        method: 'POST',
        data: {_token: "{{ csrf_token() }}", id: $id, mostrar_botao_aprovar: $mostrar_botao_aprovar},
        success: function(body){
...
function createBtReprove(obj){
    ...
    var html = "<div class=\"bt-reprove\" data-toggle=\"tooltip\" data-trigger='hover' title=\"Reprovar\" onclick=\"recusarProjeto('"+obj+"')\"></div>";

    return html;
}
function createBtAprove(mostrar_botao_aprovar, obj){
    if (mostrar_botao_aprovar === 1){
        var html = "<div class=\"bt-aprove\" data-toggle=\"tooltip\" data-trigger='hover' title=\"Aprovar\" onclick=\"aprovarProjeto('"+obj+"')\"></div>";
    }else{
        var html = '';
    }
    return html;
...
function aprovarProjeto($id){
    $.ajax({
        ...
        success: function(callback){
            message("Atenção", "Revisado com Sucesso");
            filterAjax($("#form_filter").serialize());
        },
