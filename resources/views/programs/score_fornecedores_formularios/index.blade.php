...
<form action="#" name="form_filter_pedidos_pedidos" id="form_filter_pedidos" onsubmit="return false;">
    ...
    <div class="content-buttons">
        ...
        <button name="btn-create" id="btn-create" class="btn-create">Adicionar Nova Pergunta</button>
        <button name="btn-gerenciar-grupo" id="btn-gerenciar-grupo" class="btn-create">Gerenciar Grupos</button>
    </div>
</form>	
@endsection
@section("content")
...
@section("script-footer")
	...
	$(document).ready( function () {
        ...
        table_filters_formulario_pesquisa.on('draw', function () {
            ...
            $(document).find("#btn-gerenciar-grupo").off("click");
            $(document).find("#btn-gerenciar-grupo").on("click", function(event){
                event.stopPropagation();
                showModalGerenciarGrupos();
            });
    ...
    function filtroAjax(){
        $.ajax({
            url: "{{ route("score_fornecedores_formularios.carrega_formulario") }}",
            dataType: "json",
                ...
                if(retorno.length > 0){
                    ...
                    for(var field in retorno){
                        var temp_field = [
                            ...
                            "<div><div data-toggle='tooltip' data-html='true' title='' data-original-title='" + retorno[field].grupo_pergunta + "''>" + retorno[field].grupo_pergunta + "</div></div>",
    ...
    function modalExcluirQuestao($id){
        $.ajax({
            url: "{{ Route("score_fornecedores_formularios.excluir_pergunta") }}",
            type: "POST",
            data: {
                _token: "{{csrf_token()}}",
                id: $id
    ...
    function showModalInserirQuestao(){
        $.ajax({
            url: "{{ route('score_fornecedores_formularios.modal.adicionar') }}",
            method: 'POST',
            data: {_token: "{{ csrf_token() }}"},
            success: function(body){
                createModal('modal_formulario_escore_adicionar', 'Gravar Nova Qestão', body, '');
            }
        });
    }

    function showModalGerenciarGrupos(){
        $.ajax({
            url: "{{ route('score_fornecedores_formularios.modal.gerenciar_grupos') }}",
            method: 'POST',
            data: {_token: "{{ csrf_token() }}"},
            success: function(body){
                createModal('modal_formulario_gerenciar_grupos', 'Gerenciar Grupos', body, '');
            }
        });
    }
    ...
    function showModalEditarQuestao($this){
        var $id = $($this).data("id");
        $.ajax({
            url: "{{ route('score_fornecedores_formularios.modal.editar') }}",
            method: 'POST',
            data: {_token: "{{ csrf_token() }}", id: $id},
            success: function(body){
                createModal('modal_formulario_escore_editar', 'Editar Qestão', body, '');
            }
        });
    }
