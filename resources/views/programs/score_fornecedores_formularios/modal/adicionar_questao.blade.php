...
<form action="" name="form_gravar_questao" id="form_gravar_questao" onsubmit="return false;">
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            {{ Form::select('tipo_questao', $tipo_resposta, '', ['id' => 'tipo_questao', 'class' => 'form-control', 'placeholder' => 'Selecione', 'maxlength' => '60']) }}
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            {{ Form::label('grupo', 'Grupo da Questão', []) }}
            {{ Form::select('grupo', $grupo_pergunta,'', ['id' => 'grupo', 'class' => 'form-control', 'placeholder' => 'Selecione', 'maxlength' => '60']) }}
        </div>
...
function inserirDados(data_form_modal){
    $.ajax({
        url: "{{ route('score_fornecedores_formularios.gravar_questao') }}", 
        dataType: 'json',
