...
<form action="" name="form_editar_questao" id="form_editar_questao" onsubmit="return false;">
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            {{ Form::select('tipo_questao', $tipo_resposta , $dados->score_fornecedor_formulario_tipo_respostas_id, ['id' => 'tipo_questao', 'class' => 'form-control', 'placeholder' => 'Selecione', 'maxlength' => '120']) }}
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            {{ Form::label('grupo_pergunta', 'Grupo da Questão', []) }}
            {{ Form::select('grupo_pergunta', $grupo_pergunta , $dados->score_fornecedor_formulario_grupo_perguntas_id, ['id' => 'grupo_pergunta', 'class' => 'form-control', 'placeholder' => 'Selecione', 'maxlength' => '120']) }}
        </div>
    </div>
    <div class="col-sm-12 mt-5" id="button-bottom">
        {{ Form::button('Editar', array('class' => 'btn btn-primary float-right', 'id' => 'btn-salvar')) }}
    </div> 
</form>
<script>
    ...
    function editarDados(data_form_modal){
        $.ajax({
            url: "{{ route('score_fornecedores_formularios.editar_questao') }}", 
            dataType: 'json',
