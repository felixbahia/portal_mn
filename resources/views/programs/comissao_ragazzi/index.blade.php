    ...
    <table class="table table-striped" id="table-filters">
        <thead>
            <tr>
                ...
                <th class="tb_number">Vr. Juros</th>
                <th class="tb_number">Vr. Pago</th>
                <th class="tb_number">Honorários</th>
                <th class="text_date">Data do Venc.</th>
                <th class="text_date">Data do Pag.</th>
                <th class="tb_number">% Comissão</th>
                <th class="tb_number">Vr. Comissão<a href="#" class="btn-informacao-sem-alinhamento" data-toggle="tooltip" data-placement="top" title="" data-original-title="Valor Comissão = Honorários + (Menos de 30 dias 50% do juros, se tiver juros 10% do valor pago, caso não é 5% valor pago)" style="color: black;"></a></th>
                <th class="text_date">Nota Débito</th>
                <th class="tb_number">Dias Atraso</th>
            </tr>
        </thead>
        <tbody>
        </tbody>
        <tfoot>
            <tr>
                ...
                <td class="tb_number" id="total_valor_honrarios"></td>
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('comissao_ragazzi.filtro')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            ...
            for (var fields in data.response.dados){
                temp_array = [
                    ...
                    data.response.dados[fields].numero_titulo_codigo,
                    ...
                    data.response.dados[fields].honorarios_cliente_valor,
                    data.response.dados[fields].data_vencimento,
                    data.response.dados[fields].data_pagamento,                    
                    data.response.dados[fields].porcetagem_comissao,
                    data.response.dados[fields].valor_comissao,
                    data.response.dados[fields].nota_debito,
                    data.response.dados[fields].dias_atrasos,
                ];
            ...
            $(document).find('#total_valor_honrarios').html(data.response.total_valor_honrarios);
        }
    });
}
