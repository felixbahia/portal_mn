	...
	<div class="tab-pane show active" id="pedidos" role="tabpanel" aria-labelledby="pedidos-tab">
		<div class="content-dialog-table">
			<div class="content-table">
				<table class="table table-striped table-filter-pedido-itens table-not-edit" id="table-filters">
					<thead>
						<tr>
					...
					<tbody>
						...
						<tr>
							<td><div><div data-toggle="tooltip" data-html="true" title="" data-placement="right" data-original-title="{{ $pedido["estabelecimento"] }}">{{ $pedido['estabelecimento'] }}</div></div></td>
							...
							<td><div><div data-toggle="tooltip" data-html="true" title="" data-placement="right" data-original-title="{{ $pedido["cliente"] }}">{{ $pedido['cliente'] }}</div></div></td>
							<td>{{ $pedido['terminal_numero'] }}</td>
							<td>{{ $pedido['codigo_autorizacao'] }}</td>
							<td>{{ $pedido['nsu'] }}</td>
							<td>{{ $pedido['tipo_pagamento'] }}</td>
							<td>
                                <div><div data-toggle="tooltip" data-html="true" title="" data-placement="right" data-original-title="{{ $pedido["parcela"] }}">{{ $pedido['parcela'] }}</div></div>
                            </td>
							<td>{{ $pedido['numero_cartao'] }}</td>
							<td>{{ $pedido['bandeira_cartao'] }}</td>
                            <td>
                                @if($pedido['manual'] == false)
                                    <center><i class="fa fa-check check-icon" aria-hidden="true"></i></center>
                                @else
                                    <a href="#" onclick="abrirPagamentoParcial({{ $pedido['pedido'] }})">
                                        <center><i class="bt-bloqueado" aria-hidden="true"></i></center>
                                    </a>
                                @endif
                            </td>
...
<script>
	...
	table_filters_acompanhamento_pedido = $(document).find("#table-filters").DataTable(table_filters_acompanhamento_pedido_options);
    ...
    function abrirPagamentoParcial($id){
        $.ajax({
            url: '{{ route('acompanhamento_cielo.modal.pagamentos_parciais')}}',
            type: 'POST',
            data: {
                _token: "{{ csrf_token() }}",
                id: $id,
            ...
            success: function(body){
                createModal("pagamentos_parciais", "Pagamento(s) Parcial do Pedido" + $id, body, 'modal-lg');
            }
        });
    }
</script>
