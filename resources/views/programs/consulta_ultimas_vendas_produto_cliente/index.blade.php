...
function filterAjax(form){
    ...
    limparMesagemErro();
    xhr = $.ajax({
        url: "{{ route('consulta_ultimas_vendas_produto_cliente.filter') }}",
        dataType: 'json',
        data: form.serialize(),
        method: 'POST',
        success: function(callback){
            ...
            for (var fields in data){
                temp_array = [
                    data[fields].estabelecimento,
                    data[fields].data,
                    createLinkPedido(data[fields], data[fields].pedido),
                    createLinkNota(data[fields], data[fields].nota),
                    data[fields].codigo,
                    data[fields].descricao,
                    data[fields].quantidade,
                    data[fields].preco_unitario,
                    data[fields].preco_total
                ];
