...
@section('content')
<form name="form_delete_estado_gnre" id="form_delete_estado_gnre">
    @csrf
    <div class="row">
        <div class="col-sm">
    ...
    <div class="content-buttons float-right mt-3">
        ...
        {{ Form::button('Excluir', ['id' => 'form_delete_estado_gnre_btn', 'class' => 'btn btn-danger']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_delete_estado_gnre_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('estado_gnre.excluir') }}",
                dataType: 'json',
                method: 'POST',
                data: $(document).find('#form_delete_estado_gnre').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_delete_estado_gnre').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        showErrorsInputs('#form_delete_estado_gnre', field, errors[field]);
                    }
                }
            })
        });
