...
@section('script-footer')
    $(document).ready(function(){
        ...
        table_filters= $('#table-filters-estado_gnre')
    ...
    function filtro(){
        ...
        $.ajax({
            url: "{{ route('estado_gnre.filtro') }}", 
            dataType: 'json',
            data: $(document).find('#form_estado_gnre').serialize(),
            method: 'POST',
    ...
    function modalAdicionar() {
        ...
        $.ajax({
            url: '{{ route("estado_gnre.modal.salvar") }}',
            data: {_token: '{{ csrf_token() }}'},
            method: 'POST',
            success: function(body){
                var title = 'Adicionar {{ CustomView::programaName() }}';
                createModal("modal_add_estado_gnre", title, body, '');
            },
            error: function(data){
    ...
    function modalEditar($id) {
        $.ajax({
            url: '{{ route("estado_gnre.modal.editar") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Editar {{ CustomView::programaName() }}';
                createModal("modal_edit_estado_gnre", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
    ...
    function modalDeletar($id) {
        $.ajax({
            url: '{{ route("estado_gnre.modal.excluir") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Deletar {{ CustomView::programaName() }}';
                createModal("modal_delete_estado_gnre", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
