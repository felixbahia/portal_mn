...
<form id="form_add_composicao">
    ...
    <div class="form-row float-right mt-3">
        {{ Form::button('Cadastrar', ['id' => 'form_add_composicao_btn', 'class' => 'btn btn-success']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_add_composicao_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('composicao.salvar') }}",
                dataType: 'json',
                method: 'POST', 
                data: $(document).find('#form_add_composicao').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_add_composicao').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        ...
                        showErrorsInputsAdd('#form_add_composicao', field, errors[field]);
                    }
                }
            })
        });
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $("#form_add_composicao");
