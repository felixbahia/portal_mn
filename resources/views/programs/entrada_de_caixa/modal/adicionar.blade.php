...
<form action="#" id="frm_cad_entrada_de_caixa" name="frm_cad_entrada_de_caixa" onsubmit="return false;">
    @csrf
    ...
    <div class="form-group">
        ...
        {{ Form::text('data', date('d/m/Y'), ['class' => 'form-control data']) }}
    </div>
    <div class="form-group">
        {{ Form::label('valor', 'valor') }}
        {{ Form::text('valor', '', ['class' => 'form-control valor']) }}
    </div>
...
<script>
    $(function(){
        ...
        $(document).find('#frm_cad_entrada_de_caixa').find('[type="submit"]').on("click", function(event){
            ...
            $.ajax({
                url: '{{ route("entrada_de_caixa.adicionar") }}',
                dataType: 'json',
                data: form_data,
                method: 'POST',
