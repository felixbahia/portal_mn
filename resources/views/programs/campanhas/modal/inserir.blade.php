		...
		<form action="" method="post" id="campanha_form" name="campanha_form" class="campanha_form" onsubmit="return false">
			...
			<div class="form-row">
				...
				<div class="form-group col-sm-6">
					...
					{{ Form::text('nome_campanha', '', ['id' => 'nome_campanha', 'class' => 'form-control essencial input-label', 'placeholder' => '','max-']) }}
                </div>
				<div class="form-group col-sm-3">
					...
					{!! Form::text('data_inicio_campanha', '', ['id' => 'data_inicio_campanha', 'class' => 'form-control data_modal', 'maxlength' => '20', 'readonly']) !!}
				</div>
				<div class="form-group col-sm-3">
					...
					{{ Form::text('data_fim_campanha', '', ['id' => 'data_fim_campanha', 'class' => 'form-control data_modal', 'maxlength' => '20', 'readonly']) }}
				</div>
			</div>
			...
			<div class="form-row">
				<div class="col-sm-2 col-lg-3">
					{{ Form::label('', 'Tipo de Comissão(%) Representante:', []) }}
					{{ Form::select("tipo_comissao_representante", $comissoes_tipo, "", ["id" => "estabelecimento_filtro", "class" => "form-control"]) }}
				</div>
				<div class="form-group col-sm-6 col-lg-3 content-not-estabel">
					...
					{{ Form::text('comissao_representante', '', array('id' => 'comissao_representante', 'class' => 'form-control valor text-right', "maxlength" => "180")) }}
				</div>
				<div class="col-sm-2 col-lg-3">
					{{ Form::label('', 'Tipo de Comissão(%) Vendedor Interno:', []) }}
					{{ Form::select("tipo_comissao_vendedor_interno", $comissoes_tipo, "", ["id" => "estabelecimento_filtro", "class" => "form-control "]) }}
				</div>
				<div class="form-group col-sm-6 col-lg-3 content-not-estabel">
					{{ Form::label('comissao_vendedor_interno', 'Incentivo Comissão(%) Vendedor Interno:', []) }}
					{{ Form::text('comissao_vendedor_interno', '', array('id' => 'comissao_vendedor_interno', 'class' => 'form-control valor text-right', "maxlength" => "180")) }}
				</div>
			</div>
			<div class="form-row">
				<div class="col-sm-2 col-lg-3">
					{{ Form::label('', 'Tipo de Comissão(%) Gerente:', []) }}
					{{ Form::select("tipo_comissao_gerente", $comissoes_tipo, "", ["id" => "estabelecimento_filtro", "class" => "form-control"]) }}
				</div>
				<div class="form-group col-sm-6 col-lg-3 content-not-estabel">
					{{ Form::label('comissao_gerente', 'Incentivo Comissão(%) Gerente:', []) }}
					{{ Form::text('comissao_gerente', '', array('id' => 'comissao_gerente', 'class' => 'form-control valor text-right', "maxlength" => "180")) }}
				</div>
			</div>
		    <div class="row">
				<div class="col-sm-12">
		...
		<div class="border border-dark rounded p-1 mb-1">
			<div class="row">
				<div class="form-group col-sm-3">
					...
					{!! Form::text('data_inicio_apuracao_vendedor[0]', '', ['id' => 'data_inicio_apuracao_vendedor[0]', 'class' => 'form-control data_modal data_inicio_apuracao_vendedor0', 'maxlength' => '20','style' => 'pointer-events:none', 'readonly', 'form' => 'campanha_form']) !!}
				</div>
				<div class="form-group col-sm-3">
		...
		<div class="border border-dark rounded p-1 mb-1">
			<div class="row">
				<div class="form-group col-sm-3">
					...
					{!! Form::text('data_inicio_apuracao_gerente[0]', '', ['id' => 'data_inicio_apuracao_gerente[0]', 'class' => 'form-control data_modal data_inicio_apuracao_gerente0', 'maxlength' => '20', 'readonly','style' => 'pointer-events:none', 'form' => 'campanha_form']) !!}
				</div>
				<div class="form-group col-sm-3">
	...
	<div class="tab-pane" id="inserir_produtos" role="tabpanel" aria-labelledby="inserir_produtos_tab">
		...
		<div class="content-dialog-table">
			...
			<div class="col-sm-12 mt-5" id="button-bottom">
				...
				<button type="button" id="gravar_campanha" class="btn btn-success float-right">Criar Campanha</button>
			</div>
        </div>
...
<script type="text/javascript">
	...
	function limparCamposVendedor(){
		$(document).find(".data_inicio_apuracao_vendedor0").val("");
	...
	function limparCamposGerente(){
		$(document).find(".data_inicio_apuracao_gerente0").val("");
		...
		$.ajax({
			...
			success: function(data){
				...
				for (var field in data.response.retorno){
					...
					if(verifica_duplicidade_tabela_principal <= 0 && data.response.retorno[field].ativo == '' && !produtos_enviar_array.includes(cod_produto_tratado+"|"+estabelecimento)){
						temp_line = [
							"<input type='hidden' class='class_produto"+cod_produto_tratado+data.response.retorno[field].estabelecimento+"' id='"+cod_produto_tratado+"_"+contador+"' name='produto_selecionado["+contador+"]' value='"+data.response.retorno[field].codigo+"'>"+data.response.retorno[field].codigo,
	...
	function adicionarProduto(){
		...
		data.each(function (value, index) {
			...
			var verifica_duplicidade_tabela_principal = table_produtos_enviar.rows($(document).find('.class_produto'+busca_codigo+busca_estabelecimento).parents('tr')).count();
	...
	function removerProdutoCampanha(duplicidade,codigo,estoque){
		var $class = "dialog_option_deletar";
		var $name_option_sim = "aprovar_nova_busca_sim";
		var $option_sim = "Sim";
		var $name_option_nao = "aprovar_nova_busca_nao";
		var $option_nao = "Não";
		message_sim_nao("Atenção", "Desativar o produto "+codigo+" do estoque em "+estoque+" ?", $class, $name_option_sim, $option_sim, $name_option_nao, $option_nao);
		...
		$(document).on("aprovar_nova_busca_sim", function(){
			table_produtos_enviar.rows($(document).find('.class_produto'+duplicidade).parents('tr')).remove().draw();
			var produto_busca = codigo+"|"+estoque.replaceAll(' ','');
	...
	function adicionarProdutoTemporario($this){
		...
		if(verificar_duplicidade || produtos_enviar_array.includes(duplicidade_array)){
	...
	//		"<center><a href='#' onclick=removerProdutoCampanha('"+duplicidade+"','"+codigo+"','"+estabelecimento+"') class='bt-delete'></a></center>"
			"<center><div class='text-success "  +  duplicidade +"'>ATIVO<a href='#' class= 'bt-edit-direita' onclick=removerProdutoTemporario('"+duplicidade+"')></a></div></center>",
		...
		$.ajax({
			...
			success: function(data){
				if(data.status == 'success'){
					$(document).find('#modal_campanha_inserir').modal('hide');
					message('Atenção','<div class="text-success">Campanha gravada com sucesso.</div>');
				}else{
	...
	function importarArquivo(){
		...
		.done( function (data){
			...
			for (var field in data.response.retorno){
				...
				if(!produtos_enviar_array.includes(data.response.retorno[field].codigo)){
					temp_line = [
			...
			if(verifica_adicionado){
				message('Atenção',contador_adicionado+' Produtos foram importados para serem adicionados.');
			}else{
				message('Atenção','Nenhum produto adicionado.');
			}
		})
