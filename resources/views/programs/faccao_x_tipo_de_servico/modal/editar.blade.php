...
@section('content')
<form action="" name="form_faccao_tipo_de_servico_edt" id="form_faccao_tipo_de_servico_edt" onsubmit="return false;">
    @csrf
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            <div class="input-group">
                {{ Form::text('faccao', $dados['faccao'], ['id' => 'faccao', 'class' => 'form-control input-label', 'placeholder' => 'Facção']) }}
                ...
                {!! Form::hidden('codigo_faccao', $dados['codigo_faccao'], ['id' => 'codigo_faccao']) !!}
            </div>
        </div>
        <div class="form-group col-sm-12"> 
            {{ Form::label('tipo_de_servico', 'Tipo de Serviço', []) }}
            {!! Form::select("tipo_de_servico", $tipo_de_servicos, $dados['tipo_de_servico'], ["class"=>"form-control"]) !!}
        </div>
        <div class="form-group col-sm-12"> 
            {{ Form::label('preco', 'Preço', []) }}
            {{ Form::text('preco', $dados['preco'], ['id' => 'preco', 'class' => 'form-control', 'placeholder' => 'Preço', 'maxlength' => '10']) }}
        </div>
        <div class="form-group col-sm-12"> 
            ...
            <select name="unidade" id="unidade" class="form-control">
                ...
                @if(strcasecmp($dados['unidade'], $key) == 0)
...
<script>
    $(document).ready( function () {
        form_modal_edt = $(document).find('#form_faccao_tipo_de_servico_edt');
        form_modal_edt.find("#btn-salvar").on('click', function(){
            inserirDados(form_modal_edt.serialize());
        });

        form_modal_edt.find("#bt-search-faccao-busca").on("click", function(){
            showModalFaccao($(this).data("route"), "Lista de Facções");
        });
        form_modal_edt.find("#preco").maskMoney({thousands:'', decimal:','});
        form_modal_edt.find("#faccao").autocomplete(optionsAutoCompleteFaccao());
    });

    function inserirDados(data_form_modal_edt){
        $.ajax({
            url: "{{ route('faccao_tipo_de_servico.editar') }}", 
            dataType: 'json',
    ...
    function limparMesagemErroEdt(){      
        var form_modal_edt = $("#form_faccao_tipo_de_servico_edt");
    ...
    function showErrorsInputsEdt(form_modal_edt, input, message){
        if(input.localeCompare('faccao') == 0){
            var $input = $(form_modal_edt).find("#bt-search-faccao-busca");
            $(form_modal_edt).find("input[name='faccao']").addClass('error-input');
        }else{
    ...
    function optionsAutoCompleteFaccao(){
        ...
        return {
            ...
            select: function( event, ui ) {
                event.stopPropagation();
                $(document).find('#form_faccao_tipo_de_servico_edt').find("#faccao").val(ui.item.label);
                $(document).find('#form_faccao_tipo_de_servico_edt').find("#codigo_faccao").val(ui.item.value);
                return false;
            }
        };
    }
    ...
    function returnDadosFaccao($this){
        ...
        $(document).find('#form_faccao_tipo_de_servico_edt').find("#faccao").val($this.find("td").eq(1).text()+" - "+$this.find("td").eq(3).text());
        $(document).find('#form_faccao_tipo_de_servico_edt').find("#codigo_faccao").val($this.find("td").eq(0).text());
    }
</script>
@endsection
