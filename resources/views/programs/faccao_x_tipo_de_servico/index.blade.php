    ...
    <div class="content-fields">
        ...
        <div class="col-lg-2">
            <select name="unidade" id="unidade" class="form-control">
                <option value=''>Todas as Unidades de Medida</option>
...
<div class="content-table">
    <table class="table table-striped" id="table-filters-faccao">
        <thead>
            <tr>
                <th>Facção</th>
                <th>CNPJ</th>
...
$(document).ready( function () {
    ...
    table_filters = $('#table-filters-faccao').DataTable({
        "searching": false,
        "lengthChange": false,
        "info": false,
        "scrollX": false,
...
function showModalCreate(){
    $.ajax({
        url: '{{ route('faccao_tipo_de_servico.modal.adicionar') }}',
        method: 'GET',
        success: function(body){
            var title = 'Cadastro de {{ CustomView::programaName() }}';
            createModal('modal_faccao_tipo_de_servico_adicionar', title, body, '');
        }
    });
}
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('faccao_tipo_de_servico.filter')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            ...
            for (var fields in data.response){
                temp_array = [
                    data.response[fields].faccao,
                    data.response[fields].cnpj_cpf,
                    data.response[fields].tipo_de_servico,
                    data.response[fields].unidade,
                    data.response[fields].preco,
                    createBtnEdit("{{ route('faccao_tipo_de_servico.modal.editar') }}", data.response[fields].id),
                    createBtnDelete("{{ route('faccao_tipo_de_servico.modal.deletar') }}", data.response[fields].id)
                ];
