...
<form action="#" name="form_sazonalidade" id="form_sazonalidade" onsubmit="return false">
    @csrf
...
<div class="content-table">
    <table class="table table-striped" id="table-filters-sazonalidade">
        <thead>
            <tr>
...
@section('script-footer')
    $(document).ready(function(){
        ...
        table_filters_sazonalidade = $('#table-filters-sazonalidade')
        ...
        table_filters_sazonalidade.on('draw', function () {
    ...
    function filtro(){
        table_filters_sazonalidade.clear().draw();
        $.ajax({
            url: "{{ route('sazonalidade.filtro') }}", 
            dataType: 'json',
            data: $(document).find('#form_sazonalidade').serialize(),
            method: 'POST',
            success: function(callback){
                ...
                if(dados.length > 0){
                    ...
                    table_filters_sazonalidade.rows.add(fields_filter).draw().nodes();
                }
            },
    ...
    function modalAdicionar() {
        $.ajax({
            url: '{{ route("sazonalidade.modal.salvar") }}',
            data: {_token: '{{ csrf_token() }}'},
            method: 'POST',
            success: function(body){
                var title = 'Cadastro de {{ CustomView::programaName() }}';
                createModal("modal_add_sazonalidade", title, body, '');
            },
            error: function(data){
    ...
    function modalEditar($id) {
        $.ajax({
            url: '{{ route("sazonalidade.modal.editar") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Editar {{ CustomView::programaName() }}';
                createModal("modal_edit_sazonalidade", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
    ...
    function modalDeletar($id) {
        $.ajax({
            url: '{{ route("sazonalidade.modal.excluir") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Deletar {{ CustomView::programaName() }}';
                createModal("modal_delete_sazonalidade", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
