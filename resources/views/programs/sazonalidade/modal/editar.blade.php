...
@section('content')
<form id="form_edit_sazonalidade">
    @csrf
    <div class="form-row">
        <div class="col-sm">
    ...
    <div class="form-row float-right mt-3">
        {{ Form::button('Salvar', ['id' => 'form_edit_sazonalidade_btn', 'class' => 'btn btn-primary']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_edit_sazonalidade_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('sazonalidade.editar') }}",
                dataType: 'json',
                method: 'POST', 
                data: $(document).find('#form_edit_sazonalidade').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_edit_sazonalidade').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        ...
                        showErrorsInputsEdit('#form_edit_sazonalidade', field, errors[field]);
                    }
                }
            })
        });
    ...
    function limparMesagemErroEdit(){      
        var form_modal_edit = $("#form_edit_sazonalidade");
