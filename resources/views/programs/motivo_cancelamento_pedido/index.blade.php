...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        <div class="col-lg-4">
            {!! Form::text('motivo', '', ['id' => 'motivo', 'maxlength' => '250', 'placeholder' => 'Motivo']) !!}
        </div>
    </div>
...
@section('script-footer')
    ...
    function showModalCreate(){
        $.ajax({
            url: '{{ route('motivo_cancelamento_pedido.modal.adicionar') }}',
            method: 'POST',
    ...
    function filterAjax(data_form){
        ...
        $.ajax({
            url: "{{ route('motivo_cancelamento_pedido.filter') }}",
            dataType: 'json',
            data: data_form,
            method: 'POST',
            success: function(callback){
                ...
                if(data.length > 0){
                    ...
                    for(var field in data){
                        var temp_field = [
                            data[field].motivo,
                            createBtnEdit("{{ route('motivo_cancelamento_pedido.modal.editar') }}", data[field].id),
                            createBtnDelete("{{ route('motivo_cancelamento_pedido.modal.deletar') }}", data[field].id)
                        ];
