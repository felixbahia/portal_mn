        ...
        <div class="border border-dark rounded p-1 mb-1">
            <div class="row">
                <div class="col-sm-4">
                    ...
                    {!! Form::select('forma_pagamento['.$key.']', $forma_pagamento, $pagamento['forma_pagamento'], ['id' => 'forma_pagamento['.$key.']', 'class' => 'form-control', 'placeholder' => 'Selecione']) !!}
                </div>
                <div class="col-sm-4">
                    ...
                    {!! Form::select('bandeiras['.$key.']', $bandeiras, $pagamento['bandeira'], ['id' => 'bandeiras['.$key.']', 'class' => 'form-control', 'placeholder' => 'Selecione']) !!}
                    {{ Form::hidden("pedido_id", $pedido_id, ["id" => "pedido_id"]) }}
                    {{ Form::hidden("id_pagamento_nasajon[".$key."]", $pagamento['id_pagamento_nasajon'], ["id" => "id_pagamento_nasajon[".$key."]"]) }}
                    {{ Form::hidden("id[".$key."]", $pagamento['id'], ["id" => "id[".$key."]"]) }}
                </div>
                <div class="col-sm-4">
                    ...
                    {!! Form::text('codigo_autorizacao['.$key.']', $pagamento['autorizacao'], ['id' => 'codigo_autorizacao'.$key.']', 'class' => 'form-control', 'maxlength' => '150']) !!}
                </div>
            </div>
            <div class="row">
                <div class="col-sm-4">
                    ...
                    {!! Form::select('parcelamento['.$key.']', $parcelamentos, $pagamento['parcelamento'], ['id' => 'parcelamento['.$key.']', 'class' => 'form-control', 'placeholder' => 'Selecione']) !!}
                </div>
                <div class="col-sm-4">
                    ...
                    {!! Form::text('data_autorizacao['.$key.']', $pagamento['data_autorizacao'], ['id' => 'data_autorizacao['.$key.']', 'class' => 'form-control data_autorizacao', 'maxlength' => '20','readonly']) !!}
                </div>
                <div class="col-sm-4">
                    ...
                    {!! Form::text('valor['.$key.']', $pagamento['valor'], ['id' => 'valor['.$key.']', 'class' => 'form-control valor', 'maxlength' => '50']) !!}
                </div>
            </div>
            <div class="row">
                <div class="col-sm-12">
                    ...
                    {!! Form::text('tid['.$key.']', $pagamento['documento_cartao'], ['id' => 'tid['.$key.']', 'class' => 'form-control', 'maxlength' => '30']) !!}
                </div>
            </div>
        </div>
    @endforeach
...
<script>
    ...
    function registrarPagamentos(){
        ...
        $.ajax({
            url: '{{ route("stone_pagamentos.editar_pagamento_restante") }}',
            method: 'POST',
            data: dados,
            success: function(data){
                if(data.status == 'success'){
                    message('Atenção', data.message);
                    $(document).find('#editar-pagamentos').modal('hide');
                    buscarPedidos();
