                ...
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <select name="coluna" id="coluna">
                            <option value="coluna_a">Comissão 3%</option>
                            <option value="coluna_b">Comissão 4%</option>
                            <option value="coluna_c">Comissão 5%</option>
...
<div class="content-table">
    ...
    <table class="table table-striped" id="table-filters-precos">
        <thead>
            ...
            <tr>
                @if (Auth::user()->tipo_usuario_id != 16)
                <th class='comissoes' id="coluna_a">3%</th>
                <th class='comissoes' id="coluna_b">4%</th>
                <th class='comissoes' id="coluna_c">5%</th>
                @endif
...
@section('script-footer')
    ...
    function selectEstado(origem){
        ...
        if (origem.length > 0){
            ...
            $.ajax({
                url: "{{ route('nova_lista_precos.aliquotas') }}",
                dataType: 'json',
    ...
    function filterAjax(data_form){
        ...
        $.ajax({
            url: "{{ route('nova_lista_precos.filter') }}",
            dataType: 'json',
            data: data_form,
            method: 'POST',
            success: function(data){
    ...
    function download(){
        ...
        if($("#segmentos").val() == '' && $("#grupo").val() == '' && $("#produto").val() == '' && $("#nome").val() == '' && $("#marca").val() == '' && $("#linha").val() == ''){

            msg_erro['segmentos'] = 'Pelo menos um destes valores é necessário';
            msg_erro['grupo'] = 'Pelo menos um destes valores é necessário';
            msg_erro['nome'] = 'Pelo menos um destes valores é necessário';
            msg_erro['produto'] = 'Pelo menos um destes valores é necessário';
            msg_erro['marca'] = 'Pelo menos um destes valores é necessário';
            msg_erro['linha'] = 'Pelo menos um destes valores é necessário';
        ...
        if(Object.keys(msg_erro).length > 0){
            ...
            $('<form action="{{ route('nova_lista_precos.download') }}" method="POST" target="_blank">\
                ...
                <input type="hidden" name="segmentos" value="'+$("#segmentos").val()+'" />\
