...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        ...
        <div class="col-lg-3">
            <select name="submodulo" id="submodulo">
                <option value="">Sub-Módulo</option>
            </select>
        </div>
    </div>
...
<div class="content-table">
    <table class="table table-striped table-filter-programas" id="table-filters">
        <thead>
            <tr>
                <th>Nome</th>
                <th>Módulo</th>
...
@section('script-footer')
    $(document).ready( function () {
        ...
        $("#modulo").on('change', function(event){
            ...
            if($modulo == ""){
                $("#submodulo").html("");
                $("#submodulo").append("<option value=\"\">Sub-Módulo</option>");
                return false;
            }
            $.ajax({
                ...
                success: function(data){
                    if(data.submodulos){
                        ...
                        for(var field in submodulos){
                            $("#submodulo").append("<option value=\""+submodulos[field].id+"\">"+submodulos[field].nome+"</option>");
                        }
                    }
                },
                error: function(data){
    ...
    function ajaxForm($model){
        $($model).find("#modulos_id").on('change', function(event){
            ...
            var $modulo = $this.val();
            if($modulo == ""){
                $($model).find("#sub_modulos_id").html("");
                $($model).find("#sub_modulos_id").append("<option value=\"\">Sub-Módulo</option>");
                return false;
            }
            $.ajax({
                url: "{{ route('submodulos.modulo_id') }}",
                ...
                success: function(data){
                    if(data.submodulos){
                        $($model).find("#sub_modulos_id").html("");
                        $($model).find("#sub_modulos_id").append("<option value=\"\"></option>");
                        var submodulos = data.submodulos;
                        for(var field in submodulos){
                            $($model).find("#sub_modulos_id").append("<option value=\""+submodulos[field].id+"\">"+submodulos[field].nome+"</option>");
                        }
                    }
                },
                error: function(data){
