...
function showModalCreate(){
    $.ajax({
        url: '{{ route('importacao.modal.adicionar') }}',
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}'
        },
        success: function(body){
            ...
            createModal('modal_importacao_adicionar', title, body, 'modal-lg');
        }
    });
}
        ...
        success: function(data){
            ...
            for (var fields in data.response){
                temp_array = [
                    ...
                    createBtnEdit("{{ route('importacao.modal.editar') }}", data.response[fields].id, "PCMN: "+data.response[fields].pcmn + " Proforma: " + data.response[fields].proforma + " Fornecedor: " + data.response[fields].fornecedor),
                    createBtnDelete("{{ route('importacao.modal.deletar') }}", data.response[fields].id)
                ];
...
function createBtView($value, $title){
    ...
    html = "<a href=\"#\" class=\"bt-view\" data-toggle='tooltip' data-html='true' data-id=\""+$value.id+"\" data-title=\""+$title+"\" title='Visualizar' onclick=\"abrirProjeto($(this))\"></a>";
