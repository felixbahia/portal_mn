...
<form action="" name="form_despesa_planejada" id="form_despesa_planejada" onsubmit="return false;">
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            {{ Form::text('documento', $dados['documento'], ['id' => 'documento', 'class' => 'form-control', 'placeholder' => 'Documento']) }}
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-sm-12">
            {{ Form::label('valor', 'Valor U$', []) }}
            {!! Form::text('valor', $dados['valor'], ['id' => 'valor', 'class' => 'form-control moeda text-right', 'placeholder' => 'Valor U$']) !!}
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-sm-12">
            {{ Form::label('taxa_cambio', 'Taxa Câmbio R$', []) }}
            {!! Form::text('taxa_cambio', $dados['taxa_cambio'], ['id' => 'taxa_cambio', 'class' => 'form-control decimal_quatro_casas text-right', 'placeholder' => 'Taxa Câmbio R$']) !!}
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            {{ Form::label('vencimento', 'Vencimento', []) }}
            {{ Form::text('vencimento', $dados['vencimento'], ['id' => 'vencimento', 'class' => 'form-control data', 'placeholder' => 'Vencimento']) }}
        </div>
    </div>
...
<script>
    $(document).ready(function(){
        ...
        form_modal.find(".decimal_quatro_casas").maskMoney({thousands:'.', decimal:',', precision: 4});
