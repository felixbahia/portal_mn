...
<div class="container">
    <form action="" name="form_sugestao_compra_add" id="form_sugestao_compra_add" onsubmit="return false;">
    ...
    <div class="form-row">
            <div class="input-group">
                <div class="form-group col-sm-12"> 
                    {{ Form::label('cliente', 'Cliente', []) }}
                        <div class="input-group">
                            {{ Form::hidden('cliente_codigo','', ['id' => 'cliente_codigo']) }}
                            {{ Form::hidden('produto_codigo', '', ['id' => 'produto_codigo']) }}
                            {{ Form::text('cliente_nome', '', ['id' => 'cliente_nome', 'class' => 'form-control input-label', 'placeholder' => 'Cliente', 'maxlength' => '30']) }}
        ...
        <div class="input-group">
                <div class="form-group col-sm-12"> 
                    ...
                    <div class="input-group">
                        {!! Form::text('produto_descricao', '', ['id' => 'produto_descricao', 'placeholder' => 'Produto', 'class' => 'form-control input-label']) !!}
                ...
                <div class="form-group col-sm-12"> 
                   ...
                   <div class="input-group">
                       {!! Form::text('composicao', '', ['id' => 'composicao', 'placeholder' => 'Composição do Produto', 'class' => 'form-control input-label']) !!}
       ...
       <div class="input-group">
                <div class="form-group col-sm-12"> 
                    {{ Form::label('volume_produto', 'Volume', []) }}
                    {{ Form::text('volume_produto', '', ['id' => 'volume_produto', 'class' =>'form-control text-right', 'placeholder' => 'Volume dos Produtos', 'maxlength' => '10']) }}
                </div>
                <div class="form-group col-sm-12"> 
                    {{ Form::label('valor_estimado_venda', 'Preço', []) }}
                    {{ Form::text('valor_estimado_venda', '', ['id' => 'valor_estimado_venda', 'class' => 'form-control text-right', 'placeholder' => 'Valor Unitário Estimado de Venda', 'maxlength' => '10']) }}
...
<script>
    $(document).ready( function (event) {
        ...
        form_modal_add = $(document).find('#form_sugestao_compra_add');
        ...
        form_modal_add.find("#bt-search-produto").off('click');
        form_modal_add.find("#bt-search-produto").on('click', function(){
       
            showModalProduto(form_modal_add,"Lista Produtos");
        });
        form_modal_add.find("#bt-bt-search-cliente-busca").off('click');
        form_modal_add.find("#bt-search-cliente-busca").on('click', function(){
            showModalCliente(form_modal_add,"Lista Clientes");
        });
    });

    function inserirDados(data_form_modal_add){
        form_modal_add = $(document).find('#form_sugestao_compra_add');
        var formData = new FormData($(document).find('#form_sugestao_compra_add')[0]);

        $.ajax({
            url: "{{ route('sugestao_compra.adicionar') }}", 
            dataType: 'json',
            data: formData,
            processData: false,
            contentType: false,
            ...
            error: function(callback){
                errors = callback.responseJSON.error;
                    for(var field in errors){
                        ...
                        showErrorsInputsAdd('#form_sugestao_compra_add', field, errors[field]);
                    }
            }
        });
    }
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $("#form_sugestao_compra_add");
            ...
            success: function (data){
                ...
                table_filters_produtos_busca.on('draw', function () {
                    ...
                    $(document).find("#table-filters-produtos-busca").find('tbody').find("tr").on("click", function(){
                        returnDadosProduto($(this), form_modal_add);
                    });
    ...
    function returnDadosProduto($dados, form){
        ...
        $(document).find('#produto_codigo').val($dados.find("td").eq(1).text());
   
        $(document).find('#produto_descricao').val($dados.find("td").eq(2).text());
        if($(document).find('#produto_codigo').val()){
            $(document).find('#produto_codigo').prop('readonly', true);
            $(document).find('#produto_descricao').prop('readonly', true);
        }
    }
    ...
    function optionsAutoCompleteCliente($name){
        return {
            source: function (request, response) {
                request.name = $name;
                request._token = "{{ csrf_token() }}";
                $.post("{{ route('cliente.autocomplete') }}", request, response);
            },
            delay: 700,
            minLength: 3,
            select: function( event, ui ) {
            ...
            success: function (data){
                ...
                table_dialog.on('draw', function () {
                    ...
                    $(document).find("#cliente_searsh_show").find('tbody').find("tr").on("click", function(){
                        ...
                        returnDadosCliente($(this), form_modal_add);
                    });
    ...
    function optionsAutoCompleteProduto($elemento){
        ...
        return {
            source: function (request, response) {
                request._token = "{{ csrf_token() }}";
                request.name = $elemento;
                request.campo = "codigo_produto";
