...
<form action="#" name="form_familia" id="form_familia" onsubmit="return false">
    @csrf
...
<div class="content-table">
    <table class="table table-striped" id="table-filters-familia">
        <thead>
            <tr>
...
@section('script-footer')
    $(document).ready(function(){
        ...
        table_filters_familia = $('#table-filters-familia')
        ...
        table_filters_familia.on('draw', function () {
    ...
    function filtro(){
        table_filters_familia.clear().draw();
        $.ajax({
            url: "{{ route('familia.filtro') }}", 
            dataType: 'json',
            data: $(document).find('#form_familia').serialize(),
            method: 'POST',
            success: function(callback){
                ...
                if(dados.length > 0){
                    ...
                    table_filters_familia.rows.add(fields_filter).draw().nodes();
                }
            },
    ...
    function modalAdicionar() {
        $.ajax({
            url: '{{ route("familia.modal.salvar") }}',
            data: {_token: '{{ csrf_token() }}'},
            method: 'POST',
            success: function(body){
                var title = 'Cadastro de {{ CustomView::programaName() }}';
                createModal("modal_add_familia", title, body, '');
            },
            error: function(data){
    ...
    function modalEditar($id) {
        $.ajax({
            url: '{{ route("familia.modal.editar") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Editar {{ CustomView::programaName() }}';
                createModal("modal_edit_familia", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
    ...
    function modalDeletar($id) {
        $.ajax({
            url: '{{ route("familia.modal.excluir") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Deletar {{ CustomView::programaName() }}';
                createModal("modal_delete_familia", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
