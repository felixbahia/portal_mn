...
@section('content')
    <form action="#" id='form-editar-consumo' onsubmit='return false;'>
        ...
        @csrf
        {!! Form::hidden('ficha_id', $ficha_id, ['id' => 'ficha_id_modal_consulmo']) !!}
        
        <div class='tab-content'>
            <div class="row">
                <div class="col-sm-12">
                    {!! Form::label('origem_modal_consumo', 'Tipo de composição') !!}
                    {!! Form::select('origem', ['tecido' => 'Matéria Prima', 'insumo' => 'Insumo', 'servico' => 'Mão de Obra'], '', ['id' => 'origem_modal_consumo', 'class' => 'form-control', 'placeholder' => 'Escolha uma composição']) !!}
                </div>
            </div>
            <div class="row">
                <div class="col-sm-5">
                    ...
                    <div class="input-group" id="codigo_produto_group">
                        {!! Form::text('codigo_produto', '', ['class' => 'form-control', 'id' => 'codigo_produto_modal_consumo']) !!}
                ...
                <div class="col-sm-7">
                    ...
                    {!! Form::text('descricao', '', ['class' => 'form-control', 'id' => 'descricao_modal_consumo']) !!}
                </div>
            </div>
            <div class="row">
                <div class="col-sm-12">
                    {!! Form::label('consumo_unitario_modal_consumo', 'Consumo unitário') !!}
                    {!! Form::text('consumo_unitario', '', ['class' => 'form-control text-right', 'id' => 'consumo_unitario_modal_consumo']) !!}
                </div>
            </div>
            <div class="row">
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(document).find("#origem_modal_consumo").on('change', function(){
                if ($(this).val() == 'servico'){
                    $(document).find("#consumo_unitario_modal_consumo").parents('div.row').addClass('d-none');
                }
                else{
                    $(document).find("#consumo_unitario_modal_consumo").parents('div.row').removeClass('d-none');
                }
            ...
            $(document).find("#form-editar-consumo").on('submit', function(){
                ...
                $.ajax({
                    url: "{{ route('ficha_tecnica.cadastro.composicao.salvar') }}",
                    dataType: 'json',
                    data: form.serialize(),
                    method: 'POST',
                    success: function(data){
                        $(document).find('#modal-new-consumo').modal('hide');
                    ...
                    error: function(data){
                        ...
                        for(var field in errors){
                            showErrorsInputsModalComposicaoNovo(form, field, errors[field])
                        }
                    }
                });
            })
        ...
        function showErrorsInputsModalComposicaoNovo(form, input, message){
        ...
        function codigoParaNome(){
            ...
            if($(document).find("#origem_modal_consumo").val() == 'servico'){
                data.campo = 'linha',
        ...
        function optionsAutoCompleteProdutoDescricao(){
            ...
            delay: 700,
                ...
                open: function( event, ui ){
                    $('.ui-autocomplete').css("z-index", (parseInt($('#modal-new-consumo').css('z-index')) + 1));
                },
