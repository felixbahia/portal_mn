...
<div class="row">
	<div class="col-sm-12">
		<b>Deseja realmente excluir este Cliente?</b>
	</div>
</div>
<br>
<div class="row border-bottom">
	<div class='col-sm-6'><b>{{ $dados['fisica_juridica'] == "f" ? "Nome" : "Razão" }}</b></div>
	<div class='col-sm-6'>{!! $dados['nome_razao'] !!}</div>
</div>
<div class="row border-bottom">
	<div class='col-sm-6'><b>{{ $dados['fisica_juridica'] == "f" ? "CFP" : "CNPJ" }}</b></div>
	<div class='col-sm-6'>{!! $dados['cpf_cnpj'] !!}</div>
</div>
<br>
...
<script type="text/javascript">
	$(document).ready(function($) {
		...
		$(document).find("#btn-delete").on("click", function(event) {
			var $this = $(this);
			$.ajax({
				url: "{{ route('cliente_novo.destroy') }}",
				dataType: 'json',
				...
				success: function(callback){
					if(callback.status === "success"){
						...
						message("Atenção", "Cliente novo excluido com sucesso!");
					} else {
