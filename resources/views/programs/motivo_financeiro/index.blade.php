...
@section('script-footer')
    ...
    function showModalCreate(){
        $.ajax({
            url: '{{ route('motivo_financeiro.modal.adicionar') }}',
            method: 'POST',
            data: {_token: "{{ csrf_token() }}"},
            success: function(body){
            	var title = 'Cadastro de {{ CustomView::programaName() }}';
    			createModal('modtal_motivo_financeiro_adicionar', title, body, "");
            	var modal = $("#modtal_motivo_financeiro_adicionar");
            }
        });
    }
    ...
    function filterAjax(data_form){
        ...
        $.ajax({
            url: "{{ route('motivo_financeiro.filter') }}",
            dataType: 'json',
            data: data_form,
            method: 'POST',
            success: function(callback){
                ...
                if(data.length > 0){
                    ...
                    for(var field in data){
                        var temp_field = [
                            data[field].motivo,
                            createBtnEdit("{{ route('motivo_financeiro.modal.editar') }}", data[field].id),
                            createBtnDelete("{{ route('motivo_financeiro.modal.deletar') }}", data[field].id)
                        ];
