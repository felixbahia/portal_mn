...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col">
            @if(!empty($retorno['data_pedido_nasajon']))
            <b>Emissão do Pedido</b><br>
            {{ $retorno['data_pedido_nasajon'] }}
            @endif
        </div>
    </div>
    @if(!empty($retorno['id_nota']))
    <div class="row">
        <div class="col">
            <b>Nota Fiscal</b><br>
    ...
    <script>
        ...
        function showModalCheques($id){
            ...
            $.ajax({
                url: '{{ route('cliente.posicao_sintetica.modal.titulo_cheques_vinculados') }}',
                method: 'POST',
                data: {
                    _token: '{{csrf_token()}}',
                    id: $id
