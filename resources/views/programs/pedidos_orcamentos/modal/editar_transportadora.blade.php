...
@section('content')
<form id="form_edit_transportadora_pedido" name="form_edit_transportadora_pedido">
    @csrf
...
<script>
    ...
    function editarTransportadora(){
        form = $(document).find("#form_edit_transportadora_pedido");
        $.ajax({
            url: '{{ route("pedidos_orcamentos.alterar_transportadora") }}',
            data: form.serialize(),
            method: 'POST',
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $(document).find("#form_edit_transportadora_pedido");
