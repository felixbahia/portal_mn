...
<form action="#" id="frm_edit_status" name="frm_edit_status" onsubmit="return false;">
    @csrf
...
<script type="text/javascript">
	$(function(){
        $(document).find('#frm_edit_status').find("#bt_salvar").off('click');
        $(document).find('#frm_edit_status').find("#bt_salvar").on('click', function(event){
            event.stopPropagation();
            var form = $(document).find('#frm_edit_status');
            $.ajax({
                url: '{{ route("status_projeto.editar")}}',
                dataType: 'json',
                data: form.serialize(),
                method: 'POST',
                success: function(callback){
                    if(callback.status === "success"){
                        $(document).find('#frm_edit_status').parents(".modal").modal("hide");
