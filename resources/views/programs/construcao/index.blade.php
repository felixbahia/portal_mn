...
<form action="#" name="form_construcao" id="form_construcao" onsubmit="return false">
    @csrf
...
<div class="content-table">
    <table class="table table-striped" id="table-filters-construcao">
        <thead>
            <tr>
...
@section('script-footer')
    $(document).ready(function(){
        ...
        table_filters_construcao = $('#table-filters-construcao')
        ...
        table_filters_construcao.on('draw', function () {
    ...
    function filtro(){
        table_filters_construcao.clear().draw();
        $.ajax({
            url: "{{ route('construcao.filtro') }}", 
            dataType: 'json',
            data: $(document).find('#form_construcao').serialize(),
            method: 'POST',
            success: function(callback){
                ...
                if(dados.length > 0){
                    ...
                    table_filters_construcao.rows.add(fields_filter).draw().nodes();
                }
            },
    ...
    function modalAdicionar() {
        $.ajax({
            url: '{{ route("construcao.modal.salvar") }}',
            data: {_token: '{{ csrf_token() }}'},
            method: 'POST',
            success: function(body){
                var title = 'Cadastro de {{ CustomView::programaName() }}';
                createModal("modal_add_construcao", title, body, '');
            },
            error: function(data){
    ...
    function modalEditar($id) {
        $.ajax({
            url: '{{ route("construcao.modal.editar") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Editar {{ CustomView::programaName() }}';
                createModal("modal_edit_construcao", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
    ...
    function modalDeletar($id) {
        $.ajax({
            url: '{{ route("construcao.modal.excluir") }}',
            data: {_token: '{{ csrf_token() }}', id: $id},
            method: 'POST',
            success: function(body){
                var title = 'Deletar {{ CustomView::programaName() }}';
                createModal("modal_delete_construcao", title, body, '');
            },
            error: function(data){
                message('Alerta', data.responseJSON.error.msg.user);
            }
