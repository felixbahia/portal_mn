...
@section('content')
<form name="form_delete_construcao" id="form_delete_construcao">
    @csrf
    <div class="row">
        <div class="col-sm">
    ...
    <div class="content-buttons float-right mt-3">
        ...
        {{ Form::button('Excluir', ['id' => 'form_delete_construcao_btn', 'class' => 'btn btn-danger']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_delete_construcao_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('construcao.excluir') }}",
                dataType: 'json',
                method: 'POST',
                data: $(document).find('#form_delete_construcao').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_delete_construcao').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        showErrorsInputs('#form_delete_construcao', field, errors[field]);
                    }
                }
            })
        });
