...
<form id="form_add_construcao">
    ...
    <div class="form-row float-right mt-3">
        {{ Form::button('Cadastrar', ['id' => 'form_add_construcao_btn', 'class' => 'btn btn-success']) }}
    </div>
</form>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_add_construcao_btn').on('click', function(event){
            event.stopPropagation();

            $.ajax({
                url: "{{ route('construcao.salvar') }}",
                dataType: 'json',
                method: 'POST', 
                data: $(document).find('#form_add_construcao').serialize(),
                success: function(callback){
                    if(callback.status === 'success'){
                        $(document).find('#modal_add_construcao').modal('hide');
                        filtro();
                    }
                },
                error: function(callback){
                    ...
                    for(var field in errors){
                        ...
                        showErrorsInputsAdd('#form_add_construcao', field, errors[field]);
                    }
                }
            })
        });
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $("#form_add_construcao");
