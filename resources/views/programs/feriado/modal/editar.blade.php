...
<form action="" name="form_feriado" id="form_feriado" onsubmit="return false;">
    ...
    <div class="form-row">
        <div class="form-group col-sm-12">
            ...
            {{ Form::text('feriado', $dados['feriado'], ['id' => 'feriado', 'class' => 'form-control data input-label', 'placeholder' => 'Feriado', 'maxlength' => '20']) }}
        </div>
    </div>
...
<script>
    $(document).ready( function () {
        ...
        form_modal_feriado.find('#btn-salvar').on('click',function(){
            editarDados(form_modal_feriado.serialize());
        });
    });

    function editarDados(data_form_modal_feriado){
        $.ajax({
            url: "{{ route('feriado.editar') }}", 
            dataType: 'json',
