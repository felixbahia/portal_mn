                ...
                <div class="pos-f-t">
                    <div class="collapse" id="navbarToggleExternalContent">
                        <div class="bg-light p-4">
                            <form action="#" name="form_liso" id="form_liso" onsubmit="return false">
                                @csrf
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-lg-2"> 
                                        {!! Form::select('tipo_venda', $tipo_vendas, $tipo_venda, ['id' => 'tipo_venda', 'class' => 'form-control', 'placeholder' => 'Tipo Venda']) !!}
                                    </div>
                                    <div class="col-lg-2"> 
...
@section('script-footer')
    $(document).ready( function () {
        ...
        filtroBookLiso($(document).find('#form_liso').serialize());
    ...
    function filtroBookLiso(data_form){
        form = $(document).find('#form_liso');
        $(document).find('#div-cards').html('');
        $(document).find('#footer').html('')
        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.book_lisos') }}",
            dataType: 'json',
            data: data_form,
            method: 'POST',
            success: function(callback){
                if(callback.status == 'success'){
                    if(callback.response.grupo.book_pdf.length > 0){
                        var html_grupo = '<div class="col-lg-3">'
                            +'<div class="img-container-grupo">'
                                +'<a href="#" data-grupo='+callback.response.grupo.nome+' class="ficha-tecnica"> <img class="img-thumbnail" src='+callback.response.grupo.imagem+' />'+'</a>'
                            +'</div>'
                        +'</div>'
                        +'<div class="col-lg-8">'
                            +'<div class="titulo-grupo-mobile">'
                                +'<h3>'+callback.response.grupo.nome+'</h3>'
                            +'</div>'
                            +'<div class="margin-icon">'
                                +'<div class="col-lg-0">'
                                    +'<button data-id='+callback.response.grupo.id+' data-title='+callback.response.grupo.nome+' id="button-pdf">'+'<span class="texto-button">'+'PDF'+'</span>'+'</button>'
                        ...
                        +'<div class="col-lg-8">'
                            ...
                            +'<div class="margin-icon">'
                                +'<div class="col-lg-0">'
                                    +'<button data-title='+callback.response.grupo.nome+' id="button-pdf">'+'<span class="texto-button">'+'PDF'+'</span>'+'</button>'
                                    +'<button data-id='+callback.response.grupo.id+' data-title='+callback.response.grupo.nome+' id="button-ficha">'+'<span class="texto-button">'+'FICHA&nbspTÉCNICA'+'</span>'+'</button>'
                                +'</div>'
                            +'</div>'
                        +'</div>';
                    ...
                    if(data.length > 0){
                        ...
                        html_cores += '<div class="galeria-imagem-md" id='+data[field].codigo_produto+'>'
                    ...
                    $(document).find('#div-cards').append(html_cores);
    ...
    function modalFichaTecnica($this){
        ...
        var data_form = $(document).find('#form_liso').serialize()
        xhr = $.ajax({
    ...
    function buscaFiltroFormLiso($this){
        data_form = $(document).find('#form_liso').serialize();
        form = $(document).find('#form_liso');
