    ...
    <div class="tab-pane show active" id="itens_carrinho" role="tabpanel" aria-labelledby="dados-tab">
        <form action="" name="form_visualizar_carrinho" id="form_visualizar_carrinho" onsubmit="return false;">
            <div class="content-filter-dialog">
                @foreach($dados as $value)
                    @if(!empty($value['estabelecimento']))
                        <div class="form-row mb-0" id="cancelar_pedido_{{ $value['pedido'] }}">
                            ...
                            <div class="col-sm-1">
                                <button type="button" id="bt_cancelar_pedido" data-route="{{ route('book_virtual_exibicao_new.modal.cancelar') }}" data-title="Cancelar Pedido" data-id="{{ $value['pedido'] }}" class="btn btn-danger btn-sm">Excluir</button>
                            </div>
                            <div class="col-sm-1">
                                <a href="#" id="bt_editar_transportadora" data-route="{{ route('book_virtual_exibicao_new.modal.editar_transportadora') }}" data-title="Editar Transportadora" title="Editar Transportadora" data-id="{{ $value['id'] }}" data-toggle="tooltip" data-placement="top" class="bt-edicao"></a>
                            </div>  
                        </div>
                    @endif
                @endforeach
        ...
        <div class="content-dialog-table">
                    ...
                    <tbody>
                        @foreach($filtro as $item)
                            <tr>
                                ...
                                <td>
                                    <a href='#' data-id="{{ $item['id'] }}" data-pedido="{{ $item['pedido_id'] }}" data-route="{{ route('book_virtual_exibicao_new.atualizar_carrinho') }}" data-toggle="tooltip" data-placement="top" title="Atualizar" class="bt-atualizar-carrinho"></a>
                                </td>
                                <td>
                                    <a href='#' data-id="{{ $item['id'] }}" data-route="{{ route('book_virtual_exibicao_new.modal.deletar') }}"  data-title="Deletar Produto" class="bt-delete" data-toggle="tooltip" data-placement="top" title="Excluir"></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="row my-2">
                <div class="col-auto mr-auto">
                        <button type="button" id="bt_excluir" data-route="{{ route('book_virtual_exibicao_new.modal.excluir') }}" data-title="Excluir Carrinho" data-id="{{ $value['pedido'] }}" class="btn btn-danger float-left">Excluir Todos</button>
                </div>
                <div class="col-auto">
     ...
     <div class="tab-pane" id="dados_cliente" role="tabpanel" aria-labelledby="dados-tab">
        <form action="#" method="post" id="form_dados_cliente" name="form_dados_cliente" class="cadPedido" onsubmit="return false">
            ...
            @if(empty($dados[0]['estabelecimento']))
                <div class="row">
                    <div class="col-sm-12">
                        <button type="button" id="bt_excluir_cliente" data-route="{{ route('book_virtual_exibicao_new.modal.deletar_cliente') }}" data-id="{{ $dados[0]['id'] }}" data-title="Excluir cliente" class="btn btn-danger float-right">Excluir</button>
                    </div>
                </div>
            @endif
		</form>
...
<script>
    ...
    $(document).ready( function () {
        ...
        @if(empty($dados[0]['estabelecimento'])) 
            $(document).find("#dados-cliente-tab").tab('show');
            $(document).find("#itens-carrinho-tab").addClass('disabled');
        @endif
        ...
        form_modal.find("#bt_excluir_cliente").off("click");
        $(document).find("#bt_excluir_cliente").on("click", function(e){
            e.preventDefault();
            modalDeletar($(this));
		});
    ...
    function filtro($pedido_id = null, $valor = null){
        $.ajax({
            url: '{{ route("book_virtual_exibicao_new.filtro") }}',
            type: 'POST',
            dataType: 'json',
            data: {
                _token: "{{ csrf_token() }}"
    ...
    function finalizarCompra(){
        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.finalizar') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
            },
    ...
    function deletarItem($dados){
        var $html = "<a href='#' data-id=\""+$dados.id+"\" data-route=\"{{ route('book_virtual_exibicao_new.modal.deletar') }}\"  data-title=\"Deletar Produto\" class=\"bt-delete\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Excluir\"></a>";
        return $html;
    }

    function atualizarItem($dados){
        var $html = "<a href='#' data-id=\""+$dados.id+"\" data-pedido=\""+$dados.pedido_id+"\" data-route=\"{{ route('book_virtual_exibicao_new.atualizar_carrinho') }}\"  data-title=\"Deletar Produto\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Atualizar\" class=\"bt-atualizar-carrinho\"></a>";
        return $html;
    }
