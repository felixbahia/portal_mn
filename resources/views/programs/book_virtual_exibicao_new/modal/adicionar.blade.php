			...
			<div class="form-row">
				...
				<div class="form-group col-sm-12 col-lg-6 content-not-estabel cliente_balcao condicao_pagamento">
					...
					<div class="input-group cliente_balcao" id="condicao_pagamento">
                        {{ Form::select('condicao_pagamento_descr', $pedido['condicao_pagamento_balcao'], $pedido['condicao_pagamento_codigo'], ['class' => 'form-control essencial']) }}
					</div>
					<div class="input-group cliente_balcao" id="condicao_pagamento_blacklist">
                        {{ Form::select('condicao_pagamento_descr', $pedido['condicao_pagamento_blacklist'], $pedido['condicao_pagamento_codigo'], ['class' => 'form-control essencial']) }}
					</div>
				</div>
			</div>
                ...
                <table class="table table-striped nowrap" style="width:100%" id="table-filters-pedidos-itens">
                    ...
                    @foreach($produto as $value)
                        <tr>
                            ...
                            <td>
                                <a href="#" data-toggle="tooltip" data-html="true" title="Adicionar Carrinho" data-route="{{ route('book_virtual_exibicao_new.adicionar') }}" data-estabelecimento="{{ $value['estabelecimento'] }}" data-preco_original="{{ $value['preco'] }}" data-quantidade_original="{{ $value['estoque'] }}" class="bt-carrinho-preco"></a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
...
<script type="text/javascript">
    ...
    function modalAdicionarTransportadora(){
        xhr = $.ajax({
            url: '{{ route("book_virtual_exibicao_new.modal.adicionar_transportadora") }}',
            data: {
                _token: "{{ csrf_token() }}"
            },
            method: 'POST',
    ...
    function filtroPrecos(){
        ...
        $.ajax({
            url: '{{ route('book_virtual_exibicao_new.estoque_precos') }}',
            type: 'POST',
            dataType: 'json',
            data: form.serialize(),
            success: function (data){
    ...
    function adicionarCarrinho($this, $quantidade, $preco, $transportadora = null, $transportadora_redespacho = null){
        ...
        if($(document).find('#cadPedido').find('#id').val() == $(document).find('#dados_cliente_id').val()){
            $(document).find('#cadPedido').find('#id').val('');
        }
    ...
    function adicionarCliente(){
        ...
        form = $(document).find('#cadPedido');
        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.adicionar_cliente') }}",
            type: 'POST',
            dataType: 'json',
            data: form.serialize(),
            success: function (data){
    ...
    function editarCliente(){
        ...
        form = $(document).find('#cadPedido');
        $.ajax({
            url: '{{ route('book_virtual_exibicao_new.editar_cliente') }}',
            type: 'POST',
            dataType: 'json',
            data: form.serialize(),
            success: function (data){
    ...
    function exibirCarrinho($value){
        var html = "<a href=\"#\" data-toggle=\"tooltip\" data-html=\"true\" title=\"Adicionar Carrinho\" data-route=\"{{ route('book_virtual_exibicao_new.adicionar') }}\" data-estabelecimento=\""+$value.estabelecimento+"\" data-preco_original=\""+$value.preco+"\" data-quantidade_original=\""+$value.estoque+"\" class=\"bt-carrinho-preco\"></a>"
        return html;
    }
