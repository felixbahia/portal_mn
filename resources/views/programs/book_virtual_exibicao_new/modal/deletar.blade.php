...
<form name="form_delete_carrinho" id="form_delete_carrinho">
    ...
    <div class="content" id="registro_cliente">
        <div class="row">
            <div class="col-sm">
                <h5>Deseja realmente excluir este cliente?</h5>
            </div>
        </div>
        <div class="row">
            <div class="col-sm">
                {{ Form::hidden('id_cliente', $cliente['id'], ['id' => 'id_cliente']) }}
                {{ Form::hidden('deletar_cliente', $cliente['deletar_cliente'], ['id' => 'deletar_cliente']) }}
                <b>Cliente: </b>{{ $cliente['cliente'] }} 
            </div>
        </div>
    </div>
...
<script>
    $(document).ready( function(){
        ...
        form_modal_deletar.find("#registro_cliente").hide();
        ...
        else if(form_modal_deletar.find("#deletar_cliente").val() == true){
            form_modal_deletar.find("#registro_cliente").show();
            form_modal_deletar.find("#registro_carrinho").hide();
            form_modal_deletar.find("#registro_item").hide();
            form_modal_deletar.find("#registro_pedido").hide();
        }
        ...
        form_modal_deletar.find('#form_delete_btn').on('click', function(event){
            if(form_modal_deletar.find("#deletar_item").val() == true){
                ...
                event.stopPropagation();
                deletarCliente(form_modal_deletar);
            }
        })
    });
    ...
    function deletarProduto(form_modal_deletar){
        ...
        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.deletar_item') }}",
            dataType: 'json',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    function cancelarPedido(form_modal_deletar){
        ...
        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.cancelar_pedido') }}",
            dataType: 'json',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    function excluirCarrinho(form_modal_deletar){
        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.excluir_carrinho') }}",
            dataType: 'json',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    function deletarCliente(form_modal_deletar){
        var $id = form_modal_deletar.find("#id_cliente").val();

        $.ajax({
            url: "{{ route('book_virtual_exibicao_new.deletar_cliente') }}",
            dataType: 'json',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
            ...
            success: function(body){
                $(document).find('.modal').modal('hide');
                window.location.reload();
                message("Atenção", 'Cliente excluído com sucesso!');
            },
            error: function(callback){
                errors = callback.responseJSON.message;
                message("Atenção", callback.responseJSON.message)
