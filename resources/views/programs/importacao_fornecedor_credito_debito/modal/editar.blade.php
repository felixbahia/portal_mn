...
<form action="" name="form_importacao_fornecedor_credito_debito" id="form_importacao_fornecedor_credito_debito" onsubmit="return false;">
    ...
    <div class="form-row">
        <div class="form-group col-sm-12">
            ...
            <div class="input-group">
                {{ Form::text('fornecedor', $dados['fornecedor'], ['id' => 'fornecedor', 'class' => 'form-control input-label', 'placeholder' => 'Fornecedor', 'onkeyup' => "optionsFornecedor($(this))"]) }}
    ...
    <div class="form-row">
        <div class="form-group col-sm-6"> 
            <div class="form-check">
                ...
                @else
                    {{ Form::radio('tipo', 'credito', '', ['class' => 'form-check-input', 'id'=>'tipo_credito']) }}
                @endif
        ...
        <div class="form-group col-sm-6"> 
            <div class="form-check">
                @if($dados['tipo'] == 'debito')
                    {{ Form::radio('tipo', 'debito', '', ['class' => 'form-check-input', 'id'=>'tipo_debito', 'checked']) }}
                @else 
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            {{ Form::text('valor', $dados['saldo'], ['id' => 'valor', 'class' => 'form-control text-right decimal', 'placeholder' => 'Valor']) }}
        </div>
    </div>
...
<script>
    $(document).ready( function () {
        ...
        form_importacao_fornecedor_credito_debito.find("#btn-salvar").on('click', function(){
            editarDados(form_importacao_fornecedor_credito_debito.serialize());
        });
    });

    function editarDados(data_form_importacao_fornecedor_credito_debito){
        $.ajax({
            url: "{{ route('importacao.fornecedor_credito_debito.editar') }}", 
            dataType: 'json',
    ...
    function optionsAutoCompleteFornecedor($this){
        ...
        return {
            ...
            open: function( event, ui ){
                $('.ui-autocomplete').css("z-index", (parseInt($('#modal_fornecedor_credito_debito_edit_delete').css('z-index')) + 1));
            },
