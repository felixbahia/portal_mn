        ...
        <table class="table table-striped" id="table-filters">
            <thead>
                <tr>
                    ...
                    <th class="tb_number">Nacional</th>
                    <th class="tb_number">Importado</th>
                    <th class="td_acao"></th>
                    <th class="td_acao"></th>
                </tr>
            </thead>
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="tb_number" id='total_importado'></td>
                    <td class="td_acao"></td>
                    <td class="td_acao"></td>
                </tr>
            </tfoot>
...
@section('script-footer')
    ...
    function showModalCreate(){
        $.ajax({
            url: '{{ route('faturamento_previsto.modal.adicionar') }}',
            method: 'GET',
            success: function(body){
                var title = 'Cadastro de {{ CustomView::programaName() }}';
                createModal('modal_faturamento_previsto', title, body, '');
            }
        });
    }
    ...
    function filterAjax(){
        ...
        $.ajax({
            url: '{{ route('faturamento_previsto.filtro')}}',
            data: data_form_filter,
            method: 'POST',
            success: function(data){
                linhas = [];
                
                for (var fields in data.response.faturamento_previstos){
                    temp_array = [
                        data.response.faturamento_previstos[fields].mes_ano,
                        data.response.faturamento_previstos[fields].nacional_valor,
                        data.response.faturamento_previstos[fields].importado_valor,
                        createBtnEdit("{{ route('faturamento_previsto.modal.editar') }}", data.response.faturamento_previstos[fields]),
                        createBtnDelete("{{ route('faturamento_previsto.modal.deletar') }}", data.response.faturamento_previstos[fields]),
                    ];
    ...
    function showModal($this){
        ...
        $.ajax({
            ...
            success: function(body){
                createModal('modal_faturamento_previsto_edit_delete', title, body, modal_class);
                var modal = $("#modal_faturamento_previsto_edit_delete");
            }
        });
    }
