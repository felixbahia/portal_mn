...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        <div class="col-lg-2">
            <input type="text" name="marca" id="marca" value="" placeholder="MARCA" maxlength="40"/>
        </div>
    </div>
...
function showModalCreate(){
    $.ajax({
        url: '{{ route('produto.marca.modal.adicionar') }}',
        method: 'GET',
        success: function(body){
            var title = 'Cadastro de {{ CustomView::programaName() }}';
            createModal('modal_marca_adicionar', title, body, '');
        }
    });
}
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('produto.marca.filter')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            ...
            for (var fields in data.response){
                temp_array = [
                    ...
                    createBtnEdit("{{ route('produto.marca.modal.editar') }}", data.response[fields].id),
                    createBtnDelete("{{ route('produto.marca.modal.deletar') }}", data.response[fields].id)
                ];
...
function showModal($this){
    ...
    $.ajax({
        ...
        success: function(body){
            createModal('modal_marca_edit_delete', title, body, modal_class);
            var modal = $("#modal_marca_edit_delete");
        }
    });
}
@endsection