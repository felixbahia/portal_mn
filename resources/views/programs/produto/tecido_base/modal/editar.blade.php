...
@section('content')
<form action="" name="form_tecido_base_edt" id="form_tecido_base_edt" onsubmit="return false;">
    @csrf
    ...
    <div class="form-row">
        <div class="form-group col-sm-12">
            ...
            <div class="input-group" id="cod_produto_group">
                {{ Form::text('codigo_produto', $dados['codigo_produto'], ['id' => 'codigo_produto', 'class' => 'form-control', 'placeholder' => 'Codigo Produto', 'maxlength' => '60']) }}
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            {{ Form::text('descricao', $dados['descricao'], ['id' => 'descricao', 'class' => 'form-control', 'placeholder' => 'Descrição Produto', 'maxlength' => '120']) }}
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            {{ Form::label('codigo_produto_base', 'Prefixo Produto Novo', []) }}
            {{ Form::text('codigo_produto_base', $dados['codigo_produto_base'], ['id' => 'codigo_produto_base', 'class' => 'form-control', 'placeholder' => 'Prefixo Produto Novo', 'maxlength' => '60']) }}
        </div>
    </div>
...
<script>
    $(document).ready( function () {
        form_modal = $(document).find('#form_tecido_base_edt');
    ...
    function returnDadosProdutoModal($dados, form_modal){
        ...
        form_modal.find('#descricao_produto').val($dados.find("td").eq(2).text());
    };
    ...
    function optionsAutoComplete($name){
        return {
            ...
            open: function( event, ui ){
                $('.ui-autocomplete').css("z-index", (parseInt($('#modal_tecido_base_edit_delete').css('z-index')) + 1));
            },
    ...
    function editarDados(data_form_modal){
        filterClear();
        $.ajax({
            url: "{{ route('produto.tecido_base.editar') }}", 
            dataType: 'json',
            data: data_form_modal,
            method: 'POST',
            async: false,
    ...
    function limparMesagemErroModal(){      
        var form_modal = $("#form_tecido_base_edt");
