...
@section('content')
<form action="{{ route('produto.linha.editar') }}" id="frm_edit_linha" name="frm_edit_linha" onsubmit="return false;">
    @csrf
    ...
    {!! Form::hidden('id', $dados['id']) !!}
      <div class="form-row">
        <div class="form-group  col-sm-12">
            ...
            {!! Form::text('linha', $dados['linha'], ['maxlength' => '40', 'rows' => '3', 'class' => 'form-control']) !!}
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function(){
        $(document).find('#frm_edit_linha').find("#bt_salvar").off('click');
        $(document).find('#frm_edit_linha').find("#bt_salvar").on('click', function(event){
            event.stopPropagation();
            var form = $(document).find('#frm_edit_linha');
            ...
            $.ajax({
                ...
                success: function(callback){
                    if(callback.status === "success"){
                        $(document).find('#frm_edit_linha').parents(".modal").modal("hide");
