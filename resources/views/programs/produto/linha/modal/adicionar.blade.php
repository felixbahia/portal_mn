...
@section('content')
<form action="" name="form_linha_add" id="form_linha_add" onsubmit="return false;">
    @csrf
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            {{ Form::label('linha', 'Linha', []) }}
            {{ Form::text('linha', '', ['id' => 'linha', 'class' => 'form-control', 'placeholder' => 'Nome da Linha', 'maxlength' => '40']) }}
        </div>
    </div>
...
<script>
    $(document).ready( function () {
        form_modal_add = $(document).find('#form_linha_add');
    ...
    function inserirDados(data_form_modal_add){
        $.ajax({
            url: "{{ route('produto.linha.adicionar') }}", 
            dataType: 'json',
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $("#form_linha_add");
