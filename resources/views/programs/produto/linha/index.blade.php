...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        <div class="col-lg-2">
            <input type="text" name="linha" id="linha" value="" placeholder="LINHA" maxlength="40"/>
        </div>
    </div>
...
function showModalCreate(){
    $.ajax({
        url: '{{ route('produto.linha.modal.adicionar') }}',
        method: 'GET',
        success: function(body){
            var title = 'Cadastro de {{ CustomView::programaName() }}';
            createModal('modal_linha_adicionar', title, body, '');
        }
    });
}
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('produto.linha.filter')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            ...
            for (var fields in data.response){
                temp_array = [
                    ...
                    createBtnEdit("{{ route('produto.linha.modal.editar') }}", data.response[fields].id),
                    createBtnDelete("{{ route('produto.linha.modal.deletar') }}", data.response[fields].id)
                ];
