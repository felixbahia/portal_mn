...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        <div class="col-lg-2">
            <input type="text" name="grupo" id="grupo" value="" placeholder="GRUPO" maxlength="40"/>
        </div>
    </div>
...
function showModalCreate(){
    $.ajax({
        url: '{{ route('produto.grupo.modal.adicionar') }}',
        method: 'GET',
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('produto.grupo.filter')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            ...
            for (var fields in data.response){
                temp_array = [
                    ...
                    data.response[fields].segmento,
                    createBtnEdit("{{ route('produto.grupo.modal.editar') }}", data.response[fields].id),
                    createBtnDelete("{{ route('produto.grupo.modal.deletar') }}", data.response[fields].id)
                ];
