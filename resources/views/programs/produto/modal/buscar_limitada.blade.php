...
<div class="content-filter-dialog">	
	<form action="#" name="form_filter_produtos_limitado" id="form_filter_produtos_limitado" onsubmit="return false;">
        @csrf
        {!! Form::hidden('campo', $campo, ['id' => 'campo']) !!}
        {!! Form::hidden('condicao', $condicao, ['id' => 'condicao']) !!}
...
<script>
	$(document).ready( function () {
		...
		$(document).find("#form_filter_produtos_limitado").find("#marca").autocomplete(optionsAutoCompleteModalBuscar("marca"));
        $(document).find("#form_filter_produtos_limitado").find("#linha").autocomplete(optionsAutoCompleteModalBuscar("linha"));
        $(document).find("#form_filter_produtos_limitado").find("#grupo").autocomplete(optionsAutoCompleteModalBuscar("grupo"));
        $(document).find("#form_filter_produtos_limitado").find("#descricao").autocomplete(optionsAutoCompleteDescricao());
        $(document).find("#form_filter_produtos_limitado").find("#subgrupo").autocomplete(optionsAutoCompleteModalBuscar("subgrupo"));
	...
	function filtro(){
		form_search_prod = $(document).find("#form_filter_produtos_limitado");
		data_form_search_prod = form_search_prod.serialize();
        limparMesagemErroModalBuscar();
		$.ajax({
			url: '{{ route('produto.filtersimples_limitado')}}',
	...
	function optionsAutoCompleteModalBuscar($name){
        ...
        return {
            ...
            open: function( event, ui ){
                $('.ui-autocomplete').css("z-index", (parseInt($(document).find('#form_filter_produtos_limitado').parents('.modal').css('z-index')) + 1));
            },
    ...
    function optionsAutoCompleteDescricao(){
        return {
            source: function (request, response) {
                request.campo = '{{ $campo }}';
                request.condicao = '{{ $condicao }}';
                request._token = "{{ csrf_token() }}";
            ...
            select: function( event, ui ) {
                event.stopPropagation();
                $(document).find("#form_filter_produtos_limitado").find("#descricao").val(ui.item.label);
                return false;
            }
        };
    }
    ...
    function limparMesagemErroModalBuscar(){      
        var form_modal_buscar = $("#form_filter_produtos_limitado");
