...
<div class="content-filter-dialog">	
	<form action="#" name="form_filter" id="form_filter_produtos" onsubmit="return false;">
	    ...
	    <div class="content-fields">
			...
			<div class="col-lg-2">
	            <input type="text" name="linha" id="marca_modal_linha" value="" placeholder="Linha" maxlength="250" />
	        </div>
...
<script>
	...
	table_dialog_tecido_busca = $(document).find("#table-filters-produtos-busca").DataTable(table_filters_produtos_busca);
	...
	$(document).ready(function($) {
		...
		if ($("#form_filter_produtos").find("#codigo_modal_busca").val().length > 0 || $("#form_filter_produtos").find("#nome_modal_busca").val().length > 0) {
		    table_dialog_tecido_busca.draw();
		}
		    
		$(document).find("#btn-filterform-modal").on("click", function(){

			table_dialog_tecido_busca.clear().draw();
			...
			$.ajax({
				...
				success: function(data){
					...
					table_dialog_tecido_busca.rows.add(pesquisa).draw();
