...
@section('content')
<form action="{{ route('produto.subgrupo.editar') }}" id="frm_edit_subgrupo" name="frm_edit_subgrupo" onsubmit="return false;">
    @csrf
    ...
    {!! Form::hidden('id', $dados['id']) !!}
      <div class="form-row">
        <div class="form-group  col-sm-12">
            ...
            {!! Form::text('subgrupo', $dados['subgrupo'], ['maxlength' => '40', 'rows' => '3', 'class' => 'form-control']) !!}
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function(){
        $(document).find('#frm_edit_subgrupo').find("#bt_salvar").off('click');
        $(document).find('#frm_edit_subgrupo').find("#bt_salvar").on('click', function(event){
            event.stopPropagation();
            var form = $(document).find('#frm_edit_subgrupo');
            ...
            $.ajax({
                ...
                success: function(callback){
                    if(callback.status === "success"){
                        $(document).find('#frm_edit_subgrupo').parents(".modal").modal("hide");
