...
@section('content')
<form action="" name="form_subgrupo_add" id="form_subgrupo_add" onsubmit="return false;">
    @csrf
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            {{ Form::label('subgrupo', 'Subgrupo', []) }}
            {{ Form::text('subgrupo', '', ['id' => 'subgrupo', 'class' => 'form-control', 'placeholder' => 'Nome do Subgrupo', 'maxlength' => '40']) }}
        </div>
    </div>
...
<script>
    $(document).ready( function () {
        form_modal_add = $(document).find('#form_subgrupo_add');
    ...
    function inserirDados(data_form_modal_add){
        $.ajax({
            url: "{{ route('produto.subgrupo.adicionar') }}", 
            dataType: 'json',
    ...
    function limparMesagemErroAdd(){      
        var form_modal_add = $("#form_subgrupo_add");
