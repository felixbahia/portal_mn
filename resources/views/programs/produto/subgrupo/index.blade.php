...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        <div class="col-lg-2">
            <input type="text" name="subgrupo" id="subgrupo" value="" placeholder="SUBGRUPO" maxlength="40"/>
        </div>
    </div>
...
function showModalCreate(){
    $.ajax({
        url: '{{ route('produto.subgrupo.modal.adicionar') }}',
        method: 'GET',
        success: function(body){
            var title = 'Cadastro de {{ CustomView::programaName() }}';
            createModal('modal_subgrupo_adicionar', title, body, '');
        }
    });
}
...
function filterAjax(){
    ...
    filterClear();
    $.ajax({
        url: '{{ route('produto.subgrupo.filter')}}',
        data: data_form,
        method: 'POST',
        success: function(data){
            ...
            for (var fields in data.response){
                temp_array = [
                    ...
                    createBtnEdit("{{ route('produto.subgrupo.modal.editar') }}", data.response[fields].id),
                    createBtnDelete("{{ route('produto.subgrupo.modal.deletar') }}", data.response[fields].id)
                ];
...
function showModal($this){
    ...
    $.ajax({
        ...
        success: function(body){
            createModal('modal_subgrupo_edit_delete', title, body, modal_class);
            var modal = $("#modal_subgrupo_edit_delete");
        }
    });
}
@endsection