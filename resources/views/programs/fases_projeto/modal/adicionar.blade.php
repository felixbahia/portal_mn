...
<form action="#" id="frm_cad_status" name="frm_cad_status" onsubmit="return false;">
    ...
    <div class="form-group col-sm-12">
        ...
        {!! Form::number('dias', '', ['min' => '0', 'class' => 'form-control']) !!}
    </div>
    <div class="form-group col-sm-12">
        {{ Form::label('status', 'Status') }}
        {!! Form::select('status', $status, '', ['id' => 'status', 'placeholder' => 'Selecione', 'class' => 'form-control']) !!}
    </div>
...
<script type="text/javascript">
	$(function(){
        ...
        $(document).find('#frm_cad_status').find("#bt_salvar").on('click', function(event){
            ...
            $.ajax({
                url: '{{ route("fases_projeto.adicionar")}}',
                dataType: 'json',
                data: form.serialize(),
                method: 'POST',
