...
<form action="#" name="form_filter" id="form_filter" onsubmit="return false;">
    ...
    <div class="content-fields">
        <div class="col-lg-4">
            <input type="text" name="fase" id="fases" value="" placeholder="Fase" maxlength="250" />
        </div>
    </div>
...
@section('script-footer')
    $(document).ready( function () {
        $("#btn-filterform").on("click", function(){
            filtrarFases($("#form_filter").serialize());
        });
    ...
    function showModalCreate(){
        $.ajax({
            url: '{{ route('fases_projeto.modal.adicionar') }}',
            method: 'GET',
    ...
    function showModal($this){
        ...
        $.ajax({
            ...
            success: function(body){
                createModal('modal_fases_projeto_edit', title, body, modal_class);
                var modal = $("#modal_fases_projeto_edit");
            }
        });
    }
    function filtrarFases(data_form){
        var $return;
        table_filters.clear().draw();
        $.ajax({
            url: "{{ route('fases_projeto.filtrar') }}",
            dataType: 'json',
            data: data_form,
            method: 'POST',
                ...
                if(data.length > 0){
                    ...
                    for(var field in data){
                        var temp_field = [
                            ...
                            data[field].status,
                            data[field].dias,
                            createBtnEdit("{{ route('fases_projetos.modal.editar') }}", data[field].id)
                        ];
