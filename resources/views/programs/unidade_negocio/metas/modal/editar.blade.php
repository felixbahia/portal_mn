...
<form action="" name="form_unidade_negocio_metas" id="form_unidade_negocio_metas" onsubmit="return false;">
    ...
    {!! Form::hidden('usuarios', $dados['usuarios_dados'], ['id' => 'usuarios']) !!}
    {!! Form::hidden('usuarios_unidade_negocio', '', ['id' => 'usuarios_unidade_negocio']) !!}
    {!! Form::hidden('usuarios_adicionado', $dados['usuarios_dados'], ['id' => 'usuarios_adicionado']) !!}
    <div class="form-row">
    ...
    <div class="form-row">
        <div class="form-group col-sm-12"> 
            ...
            <div class="input-group" id="unidade_negocio_group">
                {{ Form::text('unidade_negocio', $dados['unidade_negocio'], ['id' => 'unidade_negocio', 'class' => 'form-control', 'placeholder' => 'Unidade Negócio', 'maxlength' => '250']) }}
        ...
        <div class="content-table">
            <table class="table table-striped" id="table-unidade_negocio-membros">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><div><div data-toggle="tooltip" data-html="true" title="" data-original-title="{{ $usuario['nome'] }}">{{ $usuario['nome'] }}</div></div></td>
                        <td class="tb_number">{{ $usuario['meta'] }}</td>
                        <td class="td_acao"><a href="#" data-id="{{ $usuario['id_usuario'] }}" data-modal="" data-title_modal="Excluir Usuário" class="bt-delete" data-placement="top" title="Excluir" onclick="deletarRepresentante($(this))"></a></td>
                    </tr>
                @endforeach
                </tbody>
                <tfoot>
                    <td class="tb_number">Total:</td>
                    <td class="tb_number" id='total_meta'>{{ $dados['total_meta'] }}</td>
                    <td class="td_acao"></td>
                </tfoot>
            </table>
        </div>
...
<script>
    ...
    function optionsAutoCompleteUnidadeNegocioModal(form_modal){
        return {
            ...
            open: function( event, ui ){
                $('.ui-autocomplete').css("z-index", (parseInt($('#modal_unidade_negocio_edit_delete').css('z-index')) + 1));
            },
    ...
    function editarDados(form_modal){
        ...
        data_form_modal = form_modal.serialize();
        $.ajax({
            url: "{{ route('unidade_negocio.metas.editar') }}", 
            dataType: 'json',
            data: data_form_modal,
            method: 'POST',
            async: false,
