...
<form id="form_delete_cliente">
    ...
    <div class="row">
        <div class="col-sm">
            ...
            {{ $cliente->cliente_nasajon->nome . ' - ' . $cliente->cpf_cnpj }}
        </div>
    </div>
...
<script>
    $(document).ready( function(){
        ...
        $(document).find('#form_delete_cliente_btn').on('click', function(event){
            ...
            $.ajax({
                url: "{{ route('cliente_prepago.delete') }}",
                dataType: 'json',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                ...
                success: function(){
                    $(document).find('#modal_cliente_delete_prepago').modal('hide');
                    filterAjax();
                },
