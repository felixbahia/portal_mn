...
<form action="#" name="form_cliente_triangular" id="form_cliente_triangular" onsubmit="return false">
...
function filterClientesDuvidosos(){
    ...
    $.ajax({
        url: "{{ route('cliente_triangular.filtro') }}", 
        dataType: 'json',
        data: {
            _token: "{{ csrf_token() }}",
            ...
            if(dados.length > 0){
                ...
                for(var field in dados){
                    var temp_field = [
                        ...
                        createBtnDelete("{{ route('cliente_triangular.modal.deletar') }}", dados[field])
                    ];
...
function showModalCreate($id = null) {
    $.ajax({
        url: '{{ route('cliente_triangular.modal.adicionar') }}',
        data: {_token: '{{ csrf_token() }}'},
        method: 'GET',
        success: function(body){
            createModal("modal_cliente_triangular_adicionar", 'Cadastrar Cliente Triangular', body, '');
        },
        error: function(data){
...
function showModalDelete($id) {
    $.ajax({
        url: '{{ route('cliente_triangular.modal.deletar') }}',
        data: {_token: '{{ csrf_token() }}', id: $id},
        method: 'POST',
        success: function(body){
            createModal("modal_cliente_delete_duvidosos", 'Excluir Cliente Triangular', body, '');
        },
        error: function(data){
            message('Alerta', data.responseJSON.error.msg.user);
        }
