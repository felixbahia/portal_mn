...
<form action="#" id="frm_cad_cliente_limite_credito" name="frm_cad_cliente_limite_credito" onsubmit="return false;">
    ...
    <div class="form-group">
        ...
        {{ Form::text('data', $dados['data_atualizacao'], ['class' => 'form-control data', 'disabled']) }}
    </div>
    <div class="form-group">
        {{ Form::label('nome_cliente', 'Cliente') }}
        <div class="input-group" id="cod_cliente_group">
            {{ Form::text('nome_cliente', $dados['cliente'], ['id' => 'nome_cliente', 'class' => 'form-control essencial input-label', 'placeholder' => '']) }}
    ...
    <div class="form-group">
        ...
        {{ Form::text('raiz_cnpj', $dados['raiz_cnpj'], ['class' => 'form-control', 'placeholder' => '00.000.000', 'readonly']) }}
    </div>
    <div class="form-group">
        {{ Form::label('valor', 'valor') }}
        {{ Form::text('valor', $dados['valor'], ['class' => 'form-control valor', 'placeholder' => '0,00']) }}
    </div>
    <div class="form-group">
        {{ Form::label('ultima_consulta_serasa', 'Última consulta no SERASA') }}
        {{ Form::text('ultima_consulta_serasa', $dados['ultima_consulta_serasa'], ['class' => 'form-control ultima_consulta_serasa', 'placeholder' => '00/00/0000']) }}
    </div>
    <div class="form-group">
        {{ Form::label('motivo_reavaliacao', 'Motivo da reavaliação do crédito') }}
        {{ Form::text('motivo_reavaliacao', $dados['motivo_reavaliacao'], ['class' => 'form-control motivo_reavaliacao', 'maxlength' => '50']) }}
    </div>
...
<script>
    $(function(){
        ...
        $(document).find('#frm_cad_cliente_limite_credito').find('[type="submit"]').on("click", function(event){
            ...
            $.ajax({
                url: '{{ route("cliente.limite.editar") }}',
                dataType: 'json',
                data: form_data,
                method: 'POST',
