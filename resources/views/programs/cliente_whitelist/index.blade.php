...
<form action="#" name="form_cliente_whitelist" id="form_cliente_whitelist" onsubmit="return false">
...
function filterAjax(){
    ...
    $.ajax({
        url: "{{ route('cliente_whitelist.filter') }}", 
        dataType: 'json',
        data: {
            _token: "{{ csrf_token() }}",
...
function showModalCreate($id = null) {
    $.ajax({
        url: '{{ route('cliente_whitelist.modal.adicionar') }}',
        data: {_token: '{{ csrf_token() }}'},
        method: 'POST',
        success: function(body){
            createModal("modal_cliente_novo_whitelist", 'Novo Cliente Pré-pago', body, '');
        },
        error: function(data){
...
function showModalDelete($id) {
    $.ajax({
        url: '{{ route('cliente_whitelist.modal.excluir') }}',
        data: {_token: '{{ csrf_token() }}', id: $id},
        method: 'POST',
        success: function(body){
            createModal("modal_cliente_delete_whitelist", 'Excluir Cliente Pré-pago', body, '');
        },
        error: function(data){
            message('Alerta', data.responseJSON.error.msg.user);
        }
