    ...
    <table class="table table-striped table-dialog-pedido" id="table-filters-dialog">
        <thead>
            <tr>
                ...
                @if($coluna_producao)
                <th class="tb_number_150">Dias em Produção</th>
                @endif
                @if($status === 8)
                <th class="tb_date_150">Data da Finalização</th>
                @endif
        ...
        <tbody>
            @foreach ($linhas as $linha)
             <tr>
                ...
                @if($coluna_producao)
                <td class="tb_number_150">{{ $linha['dias_em_producao'] }}</td>
                @endif
                @if($status === 8)
                <td class="tb_date_150">{{ $linha['data_finalizacao'] }}</td>
                @endif
