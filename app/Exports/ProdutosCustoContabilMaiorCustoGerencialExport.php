...
class ProdutosCustoContabilMaiorCustoGerencialExport implements FromCollection
{
    /**
    ...
    */
    public function collection()
    {
		$ProdutosEstoqueObj = ProdutosEstoque::
			...
			whereHas('custoDetalhes', function($query){
				$query->where('custo_medio_contabil', '>', '0');
			})->
			get();
		$dados = collect([[
			'Código',
		...
		$ProdutosEstoqueObj->each(function($produto) use (&$dados){
			...
			if(($custo_gerencial * 1.4) >= $custo_medio){
				$dados->push([
