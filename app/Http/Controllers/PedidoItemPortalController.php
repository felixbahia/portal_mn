...
class PedidoItemPortalController extends Controller
{
    ...
    public function validarProduto($origem, $cod_produto, $estado, $aliquota, $regiao, $pedido, $preco_unitario, $quantidade, $cidade){

        $pedidoObj = PedidoPortal::with('condicao_pagamento_detalhes')->find($pedido);

        $media_prazo = $pedidoObj->condicao_pagamento_detalhes->media ?? 0;

        if (!in_array($pedidoObj->cliente->CODCAD, $this->codigo_cliente_balcao)) {
            $cepEnderecoObj = CepEndereco::whereRaw("substring(LPAD(cep::text, 8, '0'), 0, 6) = '" . substr(str_pad(preg_replace('/[_-]/','',$pedidoObj->cliente->CEP). 8, '0', STR_PAD_LEFT), 0, 5) . "'")->first();
        }

		$estabelecimentoCidadeFobObj = EstabelecimentoCidadeFob::
			where('estabelecimento', str_pad($pedidoObj->estabelecimento, '2', '0', STR_PAD_LEFT))
			->where('uf', $pedidoObj->cliente->ESTADO)
			->where('cidade', $cepEnderecoObj->cidadeBusca->cidade??null)
            ->first();

        if ($pedidoObj->tipo_frete == 'P' && is_null($estabelecimentoCidadeFobObj) && is_null($pedidoObj->transportadora_redespacho)) {
            $frete = 'cif';
        }
        else{
            $frete = 'fob';
        ...
        $valor_minimo = ceil((str_replace(",", ".", str_replace(".", "", $arr_result[0]['coluna_a'])) * (1 - $parametrosPedidoObj->valor_minimo_porcentagem/100) ) * 100)/100;
        $valor_maximo = ceil((str_replace(",", ".", str_replace(".", "", $arr_result[0]['coluna_c'])) * (1 + $parametrosPedidoObj->valor_maximo_porcentagem/100) ) * 100)/100;
        
        if(!in_array($pedidoObj->cod_cliente, $this->codigo_grupo_textil_mn)){
            if (floatval($valor_minimo) > floatval(str_replace(",", ".",  str_replace(".", "", $preco_unitario)))){
                $preco_unitario_erro = 'O preço unitário do item, está abaixo do mínimo permitido. Favor verificar';
            }
        }

        if (floatval($valor_maximo) < floatval(str_replace(",", ".", str_replace(".", "", $preco_unitario)))){
        ...
        $requestObj = new Request(['cod_cliente' => $pedidoObj->cod_cliente, 
            'codigo' => strtoupper($cod_produto),
        ...
        $produtoObj = Produto::find(strtoupper($cod_produto));

         try {
            // $result_estoque = ($infoProduto->retornaInformacoesPreco($requestObj));
            $result_estoque = $infoProduto->filterAnalise($requestObj, false, true);
        ...
        if(empty($result_estoque['data'])){
            ...
            ProdutosSemEstoque::create([
                ...
                'user' => Auth::id(),
                'qtd' => str_replace(",", ".", str_replace('.', '',$quantidade)),
            ...
            if ($pedidoObj->pedido_futuro === true){
                ...
                if (str_replace(',', '.', str_replace('.', '', $result_estoque['data']['quinzenas'][$coluna_quinzena])) < str_replace(',', '.', str_replace('.', '', $quantidade))) {
                    $quantidade_erro = ['O produto só tem '. $result_estoque['data']['quinzenas'][$coluna_quinzena] .' ' . $produtoObj->UNIDADE_VND . ' disponível na compra da quinzena e não atenderá o pedido. Favor verificar.'];

                    ProdutosSemEstoque::create([
                        'pedido' => $pedido, 
                        'cod_produto'=> strtoupper($cod_produto), 
                        'user' => Auth::id(),
                        'qtd' => $quantidade - str_replace(',', '.', str_replace('.', '', $result_estoque['data']['quinzenas'][$coluna_quinzena])),
                        'pedido_futuro' => true,
                ...
                if (str_replace(',', '.', str_replace('.', '', $result_estoque['data']['pronta_entrega'])) < str_replace(',', '.', str_replace('.', '', $quantidade))) {

                    $quantidade_erro = ['O produto só tem '. $result_estoque['data']['pronta_entrega'] .' ' . $produtoObj->UNIDADE_VND . ' em estoque e não atenderá o pedido. Favor verificar.'];

                    ProdutosSemEstoque::create([
                        'pedido' => $pedido, 
                        'cod_produto'=> strtoupper($cod_produto), 
                        'user' => Auth::id(),
                        'qtd' => str_replace(',', '.', str_replace('.', '', $quantidade)) - str_replace(',', '.', str_replace('.', '', $result_estoque['data']['pronta_entrega'])),
                        'pedido_futuro' => false,
                        'data_pedido' => $pedidoObj->data_pedido,
                        'cliente' => $pedidoObj->cod_cliente
                    ]);
    ...
    public function adicionarProdutoNasajon(PedidoPortal $pedido, $campos){
        ...
        if(empty($produtoObj)){
            $produtoObj = ProdutoUsoConsumoNasajon::where('codigo', strtoupper($campos['produto_codigo']))->first();
        }
        if(
            is_null($produtoObj) &&
        ...
        if($preco_custo === true){
            ...
            }else*/ if (!in_array($pedido->usuario_detalhes->tipo_usuario_id, [13, 16, 19, 14])){
                $coluna_preco = '0';
                if ($coluna_a <= $preco_unitario){
        ...
        ->where('produto_codigo', $campos['produto_codigo'])
        ...
        if(!empty($produtoCampanha->produto_codigo)){
            ...
            if($pedido->usuario_detalhes->tipo_usuario_id == 12){
                $incentivo_campanha = $campanha->comissao_representante;
                if($campanha->tipo_comissao_representante == 1){
                    $tipo_comisssao = $campanha->tipo_comissao_representante;
                    $campanha_informativo = 'Comissão '.$promocao.str_replace('.', ',', $comissao_porcentagem) . '%'.' + '.parserValor($campanha->comissao_representante).'% incentivo';
                    $comissao_campanha = $comissao_porcentagem + $campanha->comissao_representante;
                }
                if($campanha->tipo_comissao_representante == 2){
                    $tipo_comisssao = $campanha->tipo_comissao_representante;
                    $campanha_informativo = 'Comissão '.parserValor($campanha->comissao_representante).'% incentivo';
                    $comissao_campanha = $campanha->comissao_representante;
                }
            }
            if(in_array($pedido->usuario_detalhes->tipo_usuario_id, [16,13])){
                $incentivo_campanha = $campanha->comissao_vendedor_interno;
                if($campanha->tipo_comissao_vendedor_interno == 1){
                    $tipo_comisssao = $campanha->tipo_comissao_vendedor_interno;
                    $campanha_informativo = 'Comissão '.$promocao.str_replace('.', ',', $comissao_porcentagem) . '%'.' + '.parserValor($campanha->comissao_vendedor_interno).'% incentivo';
                    $comissao_campanha = $comissao_porcentagem + $campanha->comissao_vendedor_interno;
                }
                if($campanha->tipo_comissao_vendedor_interno == 2){
                    $tipo_comisssao = $campanha->tipo_comissao_vendedor_interno;
                    $campanha_informativo = 'Comissão '.parserValor($campanha->comissao_vendedor_interno).'% incentivo';
                    $comissao_campanha= $campanha->comissao_vendedor_interno;
                }
            }
            if(in_array($pedido->usuario_detalhes->tipo_usuario_id, [14, 19])){
                $incentivo_campanha = $campanha->comissao_gerente;
                if($campanha->tipo_comissao_gerente == 1){
                    $tipo_comisssao = $campanha->tipo_comissao_gerente;
                    $campanha_informativo = 'Comissão '.$promocao.str_replace('.', ',', $comissao_porcentagem) . '%'.' + '.parserValor($campanha->tipo_comissao_gerente).'% incentivo';
                    $comissao_campanha = $comissao_porcentagem + $campanha->comissao_gerente;
                }
                if($campanha->tipo_comissao_gerente == 2){
                    $tipo_comisssao = $campanha->tipo_comissao_gerente;
                    $campanha_informativo = 'Comissão '.parserValor($campanha->comissao_gerente).'% incentivo';
                    $comissao_campanha = $campanha->comissao_gerente;
                }
            }
        }
        ...
        $pedido_item->comissao = is_null($comissao_campanha) ? $comissao_porcentagem : $comissao_campanha;
        $pedido_item->comissao_original = $comissao_porcentagem;
        $pedido_item->ipi_produto = empty($produtoObj->ipi)? 0 : $produtoObj->ipi;
        $pedido_item->preco_base = empty($produtoObj->precovenda)? 0.01 : $produtoObj->ipi;
        ...
        $pedido_item->campanha_id = isset($campanha) ? $campanha->id :  null;
        $pedido_item->incentivo_campanha = $incentivo_campanha;
        $pedido_item->tipo_comissao_campanha = $tipo_comisssao;
        $pedido_item->save();

        unset($comissao_campanha);
        unset($comissao_porcentagem);

        if(!empty($produtoCampanha->produto_codigo)){
            $campanhasAssociacaoPedido = CampanhasAssociacaoPedido::where('pedido_id', $pedido->id)->where('campanha_id', $produtoCampanha->campanha_id)->exists();
            if($campanhasAssociacaoPedido == false){
                $campanhasAssociacaoPedidoObj = new CampanhasAssociacaoPedido;
                $campanhasAssociacaoPedidoObj->pedido_id = $pedido->id;
                $campanhasAssociacaoPedidoObj->campanha_id = $pedido_item->estoqueTotal->where('estabelecimento', '0'.$pedido->estabelecimento)->first()->campanha->id;
                $campanhasAssociacaoPedidoObj->created_by = Auth::id();
                $campanhasAssociacaoPedidoObj->save();
            }
        }
        $ipi = '';
        if($tem_ipi === true){
        ...
        if(in_array($pedido_item->cod_produto, $this->codigo_uso_consumo)){
            $descricao = $this->codigo_uso_consumo_descricao[$pedido_item->cod_produto];
        }else{
            $descricao = empty($pedido_item->info_produtoNasjon)? $pedido_item->infoProdutoUsoConsumo->especificacao : $pedido_item->info_produtoNasjon->especificacao;
        }
        ...
        if(in_array($pedido_item->cod_produto, $this->codigo_uso_consumo)){
            $result = [
                'id' => $pedido_item->id,
                'codigo' => $pedido_item->cod_produto,
                ...
                'peso_total' => parserValor($pedido_item->quantidade * $this->codigo_uso_consumo_peso[$pedido_item->cod_produto]) . ' kg',
                ...
                'comissao' =>  $promocao.str_replace('.', ',', parserValor($pedido_item['comissao'])) . '%',
                ...
                'campanha_nome' => isset($campanha) ? $campanha->nome : '',
                'campanha_informativo' => $campanha_informativo
            ];
        }else{
            $result = [
                'id' => $pedido_item->id,
                'codigo' => empty($pedido_item->info_produtoNasjon)? utf8_encode($pedido_item->infoProdutoUsoConsumo->codigo) : utf8_encode($pedido_item->info_produtoNasjon->codigo),
                'descricao' => empty($pedido_item->especificacoes->produtoGrupo->pecas)? $descricao : $descricao.' - Pecas: '.$pedido_item->especificacoes->produtoGrupo->pecas,
                'quantidade' => "<div><div class='estoque' data-toggle='popover' data-placement='left' data-html='true' title='' data-content=''>" . parserValor($pedido_item->quantidade) . "</div></div>",
                'peso_total' => empty($pedido_item->info_produtoNasjon)? parserValor($pedido_item->quantidade * $pedido_item->infoProdutoUsoConsumo->pesoliquido) . ' kg' :  parserValor($pedido_item->quantidade * $pedido_item->info_produtoNasjon->pesoliquido) . ' kg',
                'preco_unitario' => "<div><div class='preco' data-preco_original='".parserValor($preco_original)."'  data-toggle='popover' data-placement='right' data-html='true' title='' data-content=''>" . !empty($produtoObj->ipi) && intval($campos['estabelecimento']) == 3? parserValor($pedido_item->preco_unitario * (1 + ($produtoObj->ipi/100))) : parserValor($pedido_item->preco_unitario) . $ipi .  "</div></div>",
                'valor_total' => !empty($produtoObj->ipi) && intval($campos['estabelecimento']) == 3? parserValor($pedido_item->valor_total * (1 + ($produtoObj->ipi/100))) : parserValor($pedido_item->valor_total),
                'coluna' => $pedido_item->coluna,
                'comissao' =>  $promocao.str_replace('.', ',', parserValor($pedido_item['comissao'])) . '%',
    ...
    public function adicionarProduto(PedidoItemPortalRequest $request){
        $fields = $request->only(['estabelecimento', 'produto_codigo', 'pedido', 'cliente', 'preco_unitario', 'quantidade', 'produto_codigo_base', 'produto_codigo_desenho', 'produto_descricao']);
    ...
    public function editaProduto(PedidoItemPortalRequest $request){
        $fields = $request->only('id', 'produto_codigo','cliente', 'estabelecimento', 'pedido', 'preco_unitario', 'quantidade', 'produto_codigo_base', 'produto_codigo_desenho', 'produto_descricao');

        $pedidoObj = PedidoPortal::with('usuario_detalhes', 'condicao_pagamento_detalhes')->find($fields['pedido']);

        $this->excluirPedidoCielo($pedidoObj);

        return $this->editarProdutoNasajon($pedidoObj, $fields);
    }

    public function editarProdutoNasajon($pedidoObj, $fields){

        $pedido_item = PedidoItemPortal::with('carrinhoCompraItens')->findOrFail($fields['id']);

        $produtoObj = ProdutoNasajon::where('codigo', strtoupper($fields['produto_codigo']))->first();

        if(empty($produtoObj)){
            $produtoObj = ProdutoUsoConsumoNasajon::where('codigo', strtoupper($fields['produto_codigo']))->first();
        }
        ...
        $produto_sem_estoque = false;
        if(in_array($pedidoObj->tipo_venda, ['producao', 'producao_triangular', 'pre_pago_producao', 'pre_pago_producao_triangular'])){
            $validacaoProdutosProducao = $this->validarProdutoNasajonProducao($pedidoObj, strtoupper($fields['produto_codigo']), strtoupper($fields['produto_codigo_base']), strtoupper($fields['produto_codigo_desenho']), $fields['preco_unitario'], $fields['quantidade'], $estadoObj->regiao, $preco_custo);
            if ($validacaoProdutosProducao['status'] === 'error'){
                return response()->json($validacaoProdutosProducao, 422);
            }else{
                $produto_sem_estoque = $validacaoProdutosProducao['response']['produto_sem_estoque'];
            ...
            $erros = $this->validarProdutoNasajon($origem, strtoupper($fields['produto_codigo']), $pedidoObj->cliente->uf, null, $estadoObj->regiao, $pedidoObj, $fields['preco_unitario'], $fields['quantidade'], $cepEnderecoObj->cidadeBusca->cidade??'', $preco_custo);
        }
        ...
        if($preco_custo === true){
            ...
            $items = new ListaDePrecosRequest([
                ...
                'promocao' => false,
                'estabelecimento' => $pedidoObj->rj_x_sp == true? 7 : $pedidoObj->estabelecimento,
                'cliente' => $pedidoObj->cod_cliente,
            ...
            /*if($perfil_acesso === 'REP.Playstation'){
                ...
                //     }
                // }
                // if(in_array(intval($pedidoObj->estabelecimento), [3, 4])){
					$desconto_permitido = 9.01;
					if($pedidoObj->cliente->cidade == 'Nova Friburgo'){
						$desconto_permitido = 14.01;
					}
        ...
        /*
		if(strtolower($produtosEspecificacoes->linha) == 'outlet' && $pedidoObj->outlet == false){
			$validadeOutlet = $this->validadeOutlet($pedidoObj);
			if($validadeOutlet == true){
				$pedidoObj->outlet = true;
				$pedidoObj->save();
			}else{
				$error['produto_codigo'] = 'Pedido não pode misturar produtos com linha outlet';
				$error_return = ['status' => 'error', 'errors' => $error];
		...
		}elseif(strtolower($produtosEspecificacoes->linha) != 'outlet' && $pedidoObj->outlet == true){
			$error['produto_codigo'] = 'Pedido não pode misturar produtos com linha outlet';
			$error_return = ['status' => 'error', 'errors' => $error];
			return response()->json($error_return, 422);
		}*/
        ...
        if($pedidoObj->usuario == 135 && in_array($pedidoObj->cod_cliente, $this->codigo_cliente_red_d_or_sao_luiz)){
            $comissao_porcentagem = 3;
        }
        ...
        $pedido_item->pedido = $pedidoObj->id;
    	$pedido_item->usuario = Auth::id();
        ...
        if(!empty($produtoCampanha->id)){
            $campanhasAssociacaoPedido = CampanhasAssociacaoPedido::where('pedido_id', $pedidoObj->id)->where('campanha_id', $produtoCampanha->campanha_id)->exists();

            if($campanhasAssociacaoPedido == false){
                $campanhasAssociacaoPedidoObj = new CampanhasAssociacaoPedido;
                $campanhasAssociacaoPedidoObj->pedido_id = $pedidoObj->id;
                $campanhasAssociacaoPedidoObj->campanha_id = $pedido_item->estoqueTotal->where('estabelecimento', '0'.$pedidoObj->estabelecimento)->first()->campanha->id;
                $campanhasAssociacaoPedidoObj->created_by = Auth::id();
                $campanhasAssociacaoPedidoObj->save();
            }
        }

        if(!isset($fields['carrinho'])){
            if(isset($pedido_item->carrinhoCompraItens)){
                $pedido_item->carrinhoCompraItens->produto_preco = $preco_unitario;
                $pedido_item->carrinhoCompraItens->produto_quantidade = parserNumber($fields['quantidade']);
                $pedido_item->carrinhoCompraItens->updated_by = Auth::id();
                $pedido_item->carrinhoCompraItens->save();
            }
        }
        ...
        $result = [
        	'id' => $pedido_item->id,
	        'codigo' => empty($pedido_item->info_produtoNasjon)? $pedido_item->infoProdutoUsoConsumo->codigo : $pedido_item->info_produtoNasjon->codigo,
	        'descricao' => empty($pedido_item->especificacoes->produtoGrupo->pecas)? $descricao : $descricao.' - Pecas: '.$pedido_item->especificacoes->produtoGrupo->pecas,
	        'quantidade' => "<div><div class='estoque' data-toggle='popover' data-placement='left' data-html='true' title='' data-content=''>" . parserValor($pedido_item->quantidade) . "</div></div>",
            'peso_total' => empty($pedido_item->info_produtoNasjon)? parserValor($pedido_item->quantidade * $pedido_item->infoProdutoUsoConsumo->pesoliquido) . ' kg' : parserValor($pedido_item->quantidade * $pedido_item->info_produtoNasjon->pesoliquido) . ' kg',
            'preco_unitario' => "<div><div class='preco' data-preco_original='".parserValor($preco_original)."'  data-toggle='popover' data-placement='right' data-html='true' title='' data-content=''>" . !empty($produtoObj->ipi) && intval($fields['estabelecimento']) == 3? parserValor($pedido_item->preco_unitario * (1 + ($produtoObj->ipi/100))) : parserValor($pedido_item->preco_unitario) . $ipi .  "</div></div>",
	        'valor_total' => !empty($produtoObj->ipi) && intval($fields['estabelecimento']) == 3? parserValor($pedido_item->valor_total * (1 + ($produtoObj->ipi/100))) : parserValor($pedido_item->valor_total),
            'coluna' => $pedido_item->coluna,
            'comissao' => $promocao.str_replace('.', ',', $pedido_item['comissao']) . '%',
            'valor_ipi' => $pedido_item->ipi_produto,
            'total_itens' => $pedidoObj->itens_pedido->count(),
            'valor_total_produtos' => parserValor($pedidoObj->valor_total->total),
            'valor_total_frete' => parserValor($pedidoObj->valor_total_frete_produto->total),
            'valor_desconto' => parserValor($pedidoObj->valor_desconto),
            'valor_total_pedido' => parserValor($pedidoObj->valor_total->total + ($pedidoObj->tipo_frete == 'C'?$pedidoObj->valor_frete:0) - $pedidoObj->valor_desconto),
            'ipi' => $ipi,
			'preco_original' => $preco_original,
            'campanha_nome' => isset($campanha) ? $campanha->nome : '',
            'campanha_informativo' => $campanha_informativo
    ...
    public function excluiProduto(Request $request){
        $field = $request->only('id', 'carrinho');
        $id = $field['id'];
        $PedidoItemPortal = PedidoItemPortal::with('pedido_portal','carrinhoCompraItens')->find($id);
        $pedidoPortal = $PedidoItemPortal->pedido_portal;

        if(in_array($pedidoPortal->status_pedido, [3, 11])){
            return response()
            ->json(
                [
                    'status' => 'error', 
                    'message' => 'Item não pode excluído. Pedido não pode ser alterado!',
                    'error' => [],
                    'response' => []
                ],
            422);
        }
		if($pedidoPortal->status_pedido != 1 && $pedidoPortal->status_pedido != 8){
			if($pedidoPortal->status_pedido == 2){
                if(isset($pedidoPortal->aprovacao)){
                    $pedidoPortal->aprovacao->delete();
                }
			}
			$pedidoPortal->status_pedido = 1;
			$pedidoPortal->updated_by = Auth::id();
			$pedidoPortal->save();
		}

        $contador = $this->lastContadorItem($PedidoItemPortal);
        $campanha_id = $PedidoItemPortal->campanha_id;
        $PedidoItemPortal->contador = $contador;
        $PedidoItemPortal->save();
        $PedidoItemPortal->delete();

        $pedidoItemPortal = PedidoItemPortal::where('pedido', $pedidoPortal->id)
        ->where('campanha_id', $campanha_id)->exists();

        if($pedidoItemPortal == false && !is_null($campanha_id)){
            $campanhasAssociacaoPedido = CampanhasAssociacaoPedido::where('pedido_id', $pedidoPortal->id)
            ->where('campanha_id', $campanha_id)->first();

            $campanhasAssociacaoPedido->deleted_by = Auth::id();
            $campanhasAssociacaoPedido->save();
            $campanhasAssociacaoPedido->delete();
        }

        if(!isset($field['carrinho'])){
            if(isset($PedidoItemPortal->carrinhoCompraItens)){
                $PedidoItemPortal->carrinhoCompraItens->deleted_by = Auth::id(); 
                $PedidoItemPortal->carrinhoCompraItens->save();
                $PedidoItemPortal->carrinhoCompraItens->delete();
            }
        }

		$pedidoPortal->load('itens_pedido');
        /*
		if(strtolower($produtosEspecificacoes->linha) == 'outlet' && $pedidoPortal->outlet == true){

			if($pedidoPortal->itens_pedido->isNotEmpty()){
				$validadeOutlet = $this->validadeOutlet($pedidoPortal);
				if($validadeOutlet == false){
					$pedidoPortal->outlet = false;
					$pedidoPortal->save();
				}
			}else{
				$pedidoPortal->outlet = false;
        ...
        $this->excluirPedidoCielo($pedidoPortal);

    	return response()->json(['success' => 'success']);
    }

    private function lastContadorItem($PedidoItemPortalObj){
        $PedidoItemPortal = PedidoItemPortal::where('pedido', $PedidoItemPortalObj->pedido)
        ->where('cod_produto', $PedidoItemPortalObj->cod_produto)
        ->withTrashed()
        ->get();
        $contador = $PedidoItemPortal->max('contador');
        $contador += 1;
        return $contador;
    }

    public function criarProdutoProducao($dados){
        $codigo = $dados['produto_codigo'];
        $codigo = strtoupper($codigo);
        $descricao = '';
        $unidademedida = UnidadeMedidaNasajon::where('descricao', 'M')->first()->unidade;
        $origem_mercadoria = '0';
        $grupo_de_inventario = '0';
        $descricao = strtoupper($dados['produto_descricao']);

        $preco_base = Preco::find($dados['produto_codigo_base']);
        $preco_desenho = Preco::find($dados['produto_codigo_desenho']);
        $precovenda = $preco_base->preco_real + $preco_desenho->preco_real;
        unset($preco_base, $preco_desenho);
        ...
        t_produto.precovenda = ".$precovenda.";
        ...
        VAR_RETURN := estoque.api_produtonovo(t_produto);
        ...
        try{
            $produtos = DB::connection('nasajon')->select($sql_insertproduto);
        }catch(\Exception $e){
        ...
        $query_produto = ProdutoEspecificacao::select()->where('codigo_produto', 'ilike', $codigo)->first();
        ...
        if(empty($query_produto)){
            ...
            $produtoEspecificacaoObj = ProdutoEspecificacao::find($codigo); 

            $precoObj = Preco::find($codigo);
            $precoObj->updated_by = Auth::id();
			$preco_antigo = $precoObj->preco_real;
        }

        $produtoEspecificacaoObj->codigo_produto = strtoupper($codigo);
        $produtoEspecificacaoObj->marca = strtoupper('Textil MN');
        $produtoEspecificacaoObj->linha = strtoupper('Estamparia digital');
        $produtoEspecificacaoObj->grupo = strtoupper('Desenho estamparia digital');
        $produtoEspecificacaoObj->subgrupo = strtoupper('Cores unicas');
        $produtoEspecificacaoObj->descricao = strtoupper($descricao);
        $produtoEspecificacaoObj->unidade = strtoupper('M');
        $produtoEspecificacaoObj->procedencia = 0;
        $produtoEspecificacaoObj->save();

        $ProdutoTecidoBaseObj = ProdutoTecidoBase::where('codigo_produto', $dados['produto_codigo_base'])->first();

        $ProdutoTecidoEstampadoObj = new ProdutoTecidoEstampado();
        $ProdutoTecidoEstampadoObj->produto_codigo_final = $codigo;
        $ProdutoTecidoEstampadoObj->produto_tecidos_bases_id = $ProdutoTecidoBaseObj->id;
        $ProdutoTecidoEstampadoObj->produto_codigo_desenho = $dados['produto_codigo_desenho'];
        $ProdutoTecidoEstampadoObj->created_by = Auth::id();
        $ProdutoTecidoEstampadoObj->save();

        $precoObj->codigo_produto = strtoupper($codigo);
        $precoObj->preco_real = $precovenda;
        $precoObj->save();
        
		$PrecosLogObj = new PrecosLog();
		$PrecosLogObj->codigo_produto = strtoupper($codigo);
		$PrecosLogObj->preco_real_antigo = $preco_antigo;
		$PrecosLogObj->preco_real_novo = (float) $precovenda;
    ...
    private function atualizaProdutos($dados){
        $codigo = $dados['produto_codigo'];
        $codigo = strtoupper($codigo);
        $precovenda = parserNumber($dados['preco_unitario']);

        $preco_base = Preco::find($dados['produto_codigo_base']);
        $preco_desenho = Preco::find($dados['produto_codigo_desenho']);
        $precos = $preco_base->preco_real + $preco_desenho->preco_real;

        unset($preco_base, $preco_desenho);
        $precoObj = Preco::find($codigo);
        $precoObj->updated_by = Auth::id();
        $precoObj->updated_at = date('Y-m-d H:i:s');
        $precoObj->preco_real = $precos;
        $precoObj->save();
    }

    public function returnDados(Request $request){
        $fields = $request->only(['id']);
        $PedidoItemObj = PedidoItemPortal::with(['pedido_portal', 'tecidosBase', 'desenho', 'especificacoes'])->findOrFail($fields['id']);
        $retorno = [];
        $retorno['final'] = [
            'codigo' => $PedidoItemObj->cod_produto,
            'descricao' => $PedidoItemObj->especificacoes->descricao
        ];
        $retorno['desenho'] = [
            'codigo' => $PedidoItemObj->desenho->codigo_produto,
            'descricao' => $PedidoItemObj->desenho->descricao
        ];
        $retorno['tecidos_base'] = [
            'codigo' => $PedidoItemObj->tecidosBase->codigo_produto,
            'descricao' => $PedidoItemObj->tecidosBase->descricao
        ];
        $retorno['preco'] = parserValor($PedidoItemObj->preco_unitario);
        $retorno['quantidade'] = parserQtd($PedidoItemObj->quantidade);


        $ProdutoControllerObj = new ProdutoController();
        $ProdutoBaseObj = ProdutoEspecificacao::where('codigo_produto', strtoupper($PedidoItemObj->tecidosBase->codigo_produto))->first();
        $result_estoque = $ProdutoControllerObj->retornarDadosEstoqueNasajon($ProdutoBaseObj, str_pad($PedidoItemObj->pedido_portal->estabelecimento, 2, "0", STR_PAD_LEFT), $PedidoItemObj->pedido_portal, false);
        $estoque_disponivel = 0;

        if(
            isset($result_estoque['pronta_entrega']) &&
            $result_estoque['pronta_entrega'] >= 0
        ){
            $estoque_disponivel = $result_estoque['pronta_entrega'];
        }
        $retorno['estoque'] = $estoque_disponivel;
        $retorno['coluna'] = $PedidoItemObj->comissao.' %';

        $retorno['id'] = $PedidoItemObj->id;
	...
	private function validadeOutlet(PedidoPortal $PedidoPortal){
		$retorno = false;
		if($PedidoPortal->itens_pedido->isNotEmpty()){
			$PedidoPortal->itens_pedido->each(function($item) use (&$retorno){
				if(strtolower($item->especificacoes->linha) == 'outlet'){
					$retorno = true;
				}
			});
		}else{
    ...
    public function excluirPedidoCielo(PedidoPortal $PedidoPortalObj){
