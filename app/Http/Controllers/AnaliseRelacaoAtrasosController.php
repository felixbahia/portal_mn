...
class AnaliseRelacaoAtrasosController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->hasPermissionTo("programas App\AnaliseRelacaoAtrasosController") === false){
            return abort(403);
        }

        $request->session()->flash('model', 'App\AnaliseRelacaoAtrasosController');
        $gerentes = [];
    	...
    	return view("programs.analise_atrasos_equipe.index", $variaveis_view);
    }
    ...
    public function filter(Request $request){
        ...
        /*if((!empty(Auth::user()->codigo_representante) && Auth::user()->codigo_representante == '998') || (isset($representante) && $representante == '998')){
            $queryAbertos = 'with titulos as(
                select codigo, saldotitulo, null as nota_id, numero, parcela, emissao as  titulo_emissao, vencimento, tem_prorrogacao, valor, multa, nome_cliente, juros, datainiciomulta, desconto, nossonumero, enviado_para_banco, banco_codigo, observacao
        ...
        $queryAbertos .= ' AND codigo not in (\'25\',\'20\', \'30\', \'TREINAMENTO\') AND cod_cliente not in (\''. implode('\',\'', $cnpj) . '\')';
    ...
    public function modalCLiente(Request $request){
        ...
        /*if((!empty(Auth::user()->codigo_representante) && Auth::user()->codigo_representante == '998') || (isset($representante) && $representante == '998')){
            $queryAbertos = 'with titulos as(
                select codigo, vencimento, cliente_cnpj as cnpj, nome_cliente, cod_cliente, saldotitulo as saldo, multa, \'\' as nota_id, numero
                from ' . $TitulosVendedor998NasajonObj->getTable() . ' b
                where "saldotitulo" > 0  and situacao = \'Aberto\'';
        }else{*/
            $queryAbertos = 'with titulos as(
        ...
        $pedidosPreObj = ($fields['titulos_pre']) ? $pedidosPreQuery->get() : collect();
      

        $pedidosPreObj = $pedidosPreObj->filter(function($pedido) use($fields){
            return isset($pedido->pedidoNasajon) && isset($pedido->pedidoNasajon->nota);
        });
        ...
        $TitulosAberto = isset($fields['titulos']) ? collect(DB::connection('nasajon')->select($queryAbertos)) : [];
        ...
        if($fields['titulos_pre']){

            $data = Carbon::now();

            $pedidosPreObj->each(function($pedido) use (&$retorno, &$total, $estabelecimentos, $fields, $data){
                ...
                if(!isset($retorno[$pedido->pedidoNasajon->cliente_detalhes->codigo])){
                    $retorno[$pedido->pedidoNasajon->cliente_detalhes->codigo] = [
                        'cliente' => $pedido->pedidoNasajon->cliente_detalhes->nome.' - '.$pedido->pedidoNasajon->cliente_detalhes->cpf_cnpj,
                        'estabelecimento' => $estabelecimentos[$pedido->pedidoNasajon->estabelecimento_codigo],
                        'aberto' => 0,
                        'vencido' => 0,
                        'total' => 0,
                        'filter' => encrypt([
                            'estabelecimento' => $pedido->pedidoNasajon->estabelecimento_codigo,
                            'gerentes' => (isset($fields['gerentes'])) ? $fields['gerentes'] : null,
                            'vendedor_representante' => (isset($fields['vendedor_representante'])) ? $fields['vendedor_representante'] : null,
                            'cliente_nome' => $fields['cliente_nome'],
                            'cod_cliente' => $pedido->pedidoNasajon->cliente_detalhes->codigo,
                            'data_inicio' => $fields['data_inicio'],
                            'data_fim' => $fields['data_fim'],
                            'titulos' =>  (isset($fields['titulos'])) ? true : null,
                            'cheque' =>  (isset($fields['cheque'])) ? true : null,
                ...
                $retorno[$pedido->pedidoNasajon->cliente_detalhes->codigo]['vencido'] += $valor_vencido;
                $retorno[$pedido->pedidoNasajon->cliente_detalhes->codigo]['total'] += $valor_vencido;
                $total['vencido'] += $valor_vencido;
                $total['total'] += $valor_vencido;
            });
        }
        unset($pedidosPreObj);
        ...
        return view("programs.analise_atrasos_equipe.modal.cliente")->with(["total" => $total, "retorno" => $retorno, "aberturageral" => $filter['aberturageral']]);
    }
    ...
    public function modalTitulos(Request $request){
        ...
        if(!empty($filter['busca'])){
            ...
            $pedidosPreObj = $pedidosPreObj->filter(function($pedido) use ($filter){
                $busca = '/.*?' . preg_quote($filter['busca'], '/') . '.*?/';
                $cliente = $pedido->pedidoNasajon->cliente_detalhes->nome . ' - ' . $pedido->pedidoNasajon->cliente_detalhes->cpf_cnpj;
                return preg_match($busca, $cliente) > 0;
            });
        ...
        if(!empty($fields['titulos_pre'])){
            $pedidosPreObj->each(function($pedido) use(&$retorno, &$cnpjCliente, $estabelecimentos, $filter)
            {

                if($filter['total'] === 'true'){
                    if(isset($pedido->pedidoNasajon->cliente_detalhes->cpf_cnpj)){
                        $cnpjCliente[] = [
                            'cnpj' =>  $pedido->pedidoNasajon->cliente_detalhes->cpf_cnpj,
                        ];
                    }
                }
        ...
        return view('programs.analise_atrasos_equipe.modal.titulos')->with(["dados"=> $retorno, "cod_cliente" => $cnpjCliente, "totalizadores" => $total]);
    }
    ...
    public function modalRepresentante(Request $request){
        ...
        /*if((!empty(Auth::user()->codigo_representante) && Auth::user()->codigo_representante == '998') || (isset($representante) && $representante == '998')){
            $queryAbertos = 'with titulos as(
                select codigo, saldotitulo, null as nota_id, numero, parcela, emissao as titulo_emissao, vencimento, tem_prorrogacao, valor, multa, nome_cliente, cliente_cnpj as cnpj, juros, datainiciomulta, desconto, nossonumero, enviado_para_banco, banco_codigo, observacao, vendedor_codigo
                from ' . $TitulosVendedor998NasajonObj->getTable() . ' b
                where saldotitulo > 0  and situacao = \'Aberto\'';
        }else{*/
            $queryAbertos = 'with titulos as(
        ...
        $TitulosAberto = (isset($fields['titulos'])) ? collect(DB::connection('nasajon')->select($queryAbertos)) : null;
        ...
        if(isset($fields['titulos_pre'])){
            ...
            $pedidosPreObj->each(function($pedido) use(&$retorno, &$total, $fields, $estabelecimentos, $cod001, $cod){
                ...
                $usuario = $pedido->pedidoNasajon->userPortal;
                $usuariorepresentante = '';
                $representante = (!empty($usuario) && $usuario->codigo_representante != '001') ? $usuariorepresentante = $usuario->name : $cod001;
                $vendedor = (!empty($usuariorepresentante)) ? $usuario->codigo_representante : $cod['codigo_representante'];
                $representante = (!empty($usuariorepresentante)) ? $usuario->codigo_representante.' - '.$usuario->name : $cod001;
                ...
                $retorno[$representante]['aberto'] += 0;
                $retorno[$representante]['vencido'] += $valor_vencido;
                $retorno[$representante]['total'] += $valor_vencido;
        ...
        return view("programs.analise_atrasos_equipe.modal.representante")->with(["total" => $total, "retorno" => $retorno, 'saidatotal' => $saidatotal, 'filter' => $filter]);
    }
    ...
    public function modalTitulosRepresentante(Request $request){
        ...
        if($filter['representante'] == 'true'){
            ...
            $pedidosPreObj = $pedidosPreObj->filter(function($pedido) use ($representantes){
                return $pedido->pedidoNasajon->vendedor_detalhes->codigo == $representantes;
            });
                ...
                if(isset($fields['cod_vendedor']) &&  $fields['cod_vendedor'] == '001'){
                    ...
                    if($vendedor == true){
                        $retorno[] = [
                            ...
                            'numero' => $titulo->chequeTitulo->pluck('titulo_numero')->filter()->implode(', '),
                            'cheque' => $titulo->numero_cheque,
                            'parcela' => '',
        ...
        if(isset($fields['titulos_pre'])){
            ...
            $pedidosPreObj->each(function($pedido) use(&$retorno, $estabelecimentos, $data){
        ...
        if(!empty($fields['titulos_pre'])){
            ...
            $valor_pago = $pedidosPreObj->sum(function($pedido) use ($data){
                return $pedido->lancamentos->sum("valor_pago");
            });
