...
class ListagemDePrecosController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if(Auth::user()->hasPermissionTo("programas App\ListagemDePrecos") === false){
            return abort(403);
        }

        $estadosObj = CepEstado::all()->toArray();

        $request->session()->flash('model', 'App\ListagemDePrecos');
        ...
        return view('programs.listagem_precos.index')->with(['origem' => $origem, 'campos_salvos' => $campos_salvos,'tem_estoque' => $tem_estoque]);
    }
    ...
    public function export(ListaDePrecosRequest $request){
        $dados = $this->filter($request, true, false, true);

        $array_coluna = ["coluna_a", "coluna_b", "coluna_c"];
        $array_prazo = ["prazo_vista", "prazo_15", "prazo_30", "prazo_45", "prazo_60"];

        $arquivo = 'lista_de_precos.xls';
        if(in_array($request->input("coluna"), $array_coluna)){
            $html = "
                    <table>
                        <tr>
                            <th colspan=\"13\" align=\"center\">Lista de Preços - Em {$request->moeda} - Origem: {$request->origem} - Alíquota de {$request->aliquota}%</th>
                        </tr>
                        <tr>
                            <th>GRUPO</th>
                            <th>NOME</th>
            ...
            foreach ($dados as $key => $value) {
                ...
                $html .= "<tr>
                        ...
                        <td>{$value["largura"]}</td>
                        <td>{$value["unidade"]}</td>
                        <td>{$value["prazo_vista"]}</td>
                        <td>{$value["prazo_15"]}</td>
                        <td>{$value["prazo_30"]}</td>
                        <td>{$value["prazo_45"]}</td>
                        <td>{$value["prazo_60"]}</td>
                        <td>{$value["prazo_90"]}</td>
                </tr>";
            }
            $html .= "</table>";
        }else{
            $html = "
                    <table>
                        <tr>
                            <th colspan=\"11\" align=\"center\">Lista de Preços - Em {$request->moeda} - Origem: {$request->origem} - Alíquota de {$request->aliquota}%</th>
                        </tr>
                        <tr>
                            <th>GRUPO</th>
                            <th>NOME</th>
            ...
            foreach ($dados as $key => $value) {
                ...
                $html .= "<tr>
                        ...
                        <td>{$value["largura"]}</td>
                        <td>{$value["unidade"]}</td>
                        <td>{$value["coluna_a"]}</td>
                        <td>{$value["coluna_b"]}</td>
                        <td>{$value["coluna_c"]}</td>
                </tr>";
            }
            $html .= "</table>";
        }
    ...
    public function filter(ListaDePrecosRequest $request, $use_auth = true, $salvar_pesquisa = true, $retornar_array = false, $aprovacao = false, $exportacao = false, $deletados = false, $olharEstoque = true){
        ...
        $fields = $request->only(['origem', 'grupo', 'produto', 'nome', 'marca', 'linha', 'aliquota', 'coluna', 'moeda', 'frete', 'estado', 'estabelecimento', 'tipo_cliente', 'prazo_medio', 'cidade', 'promocao', 'cliente', 'codigo_vendedor','tem_estoque']);
        ...
        if($olharEstoque){
            ...
            if(isset($fields['nome']) && !empty($fields['nome'])){
                ...
                if(!empty($produtoEspecificacao)){
                    $grupo_verificacao = '';
                    if(!empty($produtoEspecificacao->produtoGrupo)){
                        $grupo_verificacao = $produtoEspecificacao->produtoGrupo->descricao;
                    }
            ...
            if(isset($fields['produto']) && !empty($fields['produto'])){
                $produtoEspecificacao = ProdutoEspecificacao::with(['produtoGrupo'])->where('codigo_produto', 'ilike', '%' . $fields['produto'] . '%')->first();
                ...
                if(!empty($produtoEspecificacao)){
                    ...
                    if(trim(strtolower($grupo_verificacao)) !== 'desenho estamparia digital' ||
                        trim(strtolower($produtoEspecificacao->linha)) !== 'mao de obra' ||
                        strtolower($produtoEspecificacao->linha) !== 'insumo'){
            ...
            if($estoque){
                if(!empty($fields['tem_estoque'] )){
                    $produtoQuery->whereHas('estoque', function($query){
                        $query->where(DB::RAW('(estoque + compras)'),'>',0);
                    });
                }
            }
        }
        ...
        if(!empty($fields['tem_estoque'] )){
            $produtoQuery->whereHas('estoque', function($query){
                $query->where(DB::RAW('(estoque + compras + saldo_fiscal)'),'>',0);
            });
        }
        $produtoQuery->whereHas('especificacoes', function($query) use($fields, $deletados){
        ...
        foreach ($produtosObj as $value) {
            ...
            if ($deletados == false){
                ...
                if (!empty($valor_base)){
                    ...
                    if($estabelecimento_origem != 3 && $value->especificacoes->produto_grupos_id != 2872){
    ...
    */
    static public function exportarListas(){
        ...
        $gruposObjReal = ProdutoEspecificacao::whereHas('estoque', function($query){
        ...
        ->unique(function($item){
            return $item->marca.$item->linha.$item->grupo.$item->subgrupo.$item->preco->preco_real;
        });
        ...
        $origens = ['RO', 'SP', 'TO'];
        $colunas = ['coluna_a', 'coluna_b', 'coluna_c'];
        $fretes = ['cif', 'fob'];
        $comissao = ['coluna_a' => '3%', 'coluna_b' => '4%', 'coluna_c' => '5%'];
        ...
        $listaPrecosObj = new ListagemDePrecosController();
                ...
                foreach ($colunas as $coluna){
                    foreach ($fretes as $frete){
                        ...
                        if ($aliquota->origem == $origem &&!($origem == 'RO' && $aliquota->icms_venda != 4)){
                            ...
                            echo 'Dados importados: ' . Carbon::now()->format('d/m/Y H:i:s') . PHP_EOL . "Lista: " . $titulo . PHP_EOL . PHP_EOL;
                            ...
                            if($origem == 'RO'){
                                ...
                                foreach(['isento', 'normal'] as $tipo_cliente){
                                    ...
                                    $listas[] = [
                                        'dados' => $listaPrecosObj->filter($request, $use_auth, $salvar_pesquisa, $retornar_array, $aprovacao, $exportacao, $deletados, $olharEstoque),
                                        'filename' => storage_path() . '/app/public/listas_preco/' . $origem . '_' . $icms . '_' . $coluna . '_' . strtoupper($frete) . '_' . $aliquota->frete_adicional . ($origem == 'RO'?'_dolar':''). '.pdf',
                                        'titulo' => $titulo,
                                        'gerado' => Carbon::Now()->format('H:i:s - d/m/Y'),
                                    ];
        ...
        echo 'Importadas ' . count($listas) . ' listas - Iniciando processamento dos dados' . PHP_EOL . PHP_EOL;

        foreach ($listas as $lista){

            $pdf = PDF::loadView('pdf.listagem_precos', ['lista' => $lista['dados'], 'titulo' => $lista['titulo'], 'gerado' => $lista['gerado']], [], ['title' => $lista['titulo'], 'custom_font_dir' => '', 'custom_font_data' => []] )->save($lista['filename']);

            echo 'Gerado ' . Carbon::now()->format('d/m/Y H:i:s') . PHP_EOL;

        }

        $fim = Carbon::now();

        echo 'Demorou ' . $inicio->diffForHumans() . ' para gerar as tabelas.' . PHP_EOL;
        
    }

    function linkLista(Request $request){
        $fields = $request->only(['origem', 'estado', 'coluna', 'frete', 'moeda', 'tipo_cliente']);

        $aliquotaQuery = AliquotaPreco::
            select('origem', 'icms_venda', 'frete_adicional', 'icms_venda_cliente_isento')
            ->where('origem', $fields['origem'])
            ->where('estado', $fields['estado']);

        if($fields['origem'] == 'RO'){
            $aliquotaQuery->where('internacional', true);
        }
        else{
            $aliquotaQuery->where('internacional', false);
        }
        
        $aliquotaObj = $aliquotaQuery->first();

        if ($fields['tipo_cliente'] == 'isento'){
            $icms = $aliquotaObj->icms_venda_cliente_isento;
        }
        else{
            $icms = $aliquotaObj->icms_venda;
        }

        $frete_adicional =  $aliquotaObj->frete_adicional;

        $filename = 'public/listas_preco/' . $fields['origem'] . '_' . $icms . '_' . $fields['coluna'] . '_' . strtoupper($fields['frete']) . '_' . $aliquotaObj->frete_adicional .( $fields['origem'] == 'RO'?'_' . $fields['moeda'] : '' ) . '.pdf';
