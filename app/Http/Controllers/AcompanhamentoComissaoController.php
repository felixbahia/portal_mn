...
use App\Http\Requests\AcompanhamentoComissaoFiltroRequest;

class AcompanhamentoComissaoController extends Controller
{
    ...
    public function index(Request $request){
        if(Auth::user()->hasPermissionTo("programas App\AcompanhamentoComissao") === false){
            return abort(403);
        }
        $request->session()->flash('model', 'App\AcompanhamentoComissao');
        ...
        return view('programs.acompanhamento_comissao.index')->with(['data' => $data, 'unidades_negocios' => $unidades_negocios, 'representantes' => $representantes]);
    }
    ...
    public function filtro(AcompanhamentoComissaoFiltroRequest $request, $retorno_array = false){
        ...
        $data_verificacao = Carbon::parse('2020-06-01 00:00:00');
        $data_verificacao_hospitalar_outfits = Carbon::parse('2020-09-01 00:00:00');
    ...
    public function modalMembros(Request $request){
        ...
        foreach($vendedores as $key_unidade_negocio => $unidade_negocio){
            foreach($unidade_negocio as $codigo_vendedor => $vendedor){
                ...
                $vendedores[$key_unidade_negocio][$codigo_vendedor]['valor_equipe'] = empty($contador_vendedores_interno)? 0 : $valores_totais[$unidade_negocio_id]['valor'] * ($comissao_equipe[$unidade_negocio_id] / 100) / $contador_vendedores_interno;
                $vendedores[$key_unidade_negocio][$codigo_vendedor]['valor_total'] = $vendedores[$key_unidade_negocio][$codigo_vendedor]['valor_individual'] + $vendedores[$key_unidade_negocio][$codigo_vendedor]['valor_equipe'];
        ...
        $total_atingimento = empty($total_meta) || empty($total_valor)? '' : parserValor(($total_valor/$total_meta)*100).'%'; 
        ...
        return view('programs.acompanhamento_comissao.modal.membro')->
    ...
    public function verificacaoGeracaoIntegracaoComFolha(AcompanhamentoComissaoFiltroRequest $request){
        $fields = $request->only('mes_ano', 'unidade_negocio', 'vendedor');

        $data_escolhida_inicial = Carbon::createFromFormat('d/m/Y', '01/'.$fields['mes_ano'])->setTime(0,0,0);

        $query_verificacao_cadastro = UnidadeNegocioMeta::select();
        if(!empty($fields['unidade_negocio'])){
            $query_verificacao_cadastro->where('unidades_negocios_id', $fields['unidade_negocio']);
        }
        $query_verificacao_cadastro->where('data', $data_escolhida_inicial);
        $query_verificacao_cadastro->with(['usuarios' => function ($query) use($fields){
            ...
            $query->with(['detalhesUsuario' => function ($query) use($fields){
                if(!empty($fields['vendedor'])){
                    $query->where('codigo_representante', $fields['vendedor']);
                }
                $query->where('tipo_usuario_id', 16);
        ...
        $query_verificacao_cadastro->whereHas('usuarios', function ($query) use($fields){
            $query->whereHas('detalhesUsuario', function ($query) use($fields){
                if(!empty($fields['vendedor'])){
                    $query->where('codigo_representante', $fields['vendedor']);
                }
        ...
        $result_verificacao_cadastro = $query_verificacao_cadastro->first();

        if(empty($result_verificacao_cadastro)){
            return response()->json([
                'status' => 'error',
                'message' => 'Não há vendedor interno.',
                'error' => [],
                'response' => []
            ], 422);
        }

        $query_gerentes = UnidadeNegocio:: select();
        $result_gerentes = $query_gerentes->get();

        $query_verificacao_usuario = User::select();
        $query_verificacao_usuario->where(function($query) use($result_gerentes){
            $query->orWhere('tipo_usuario_id', 16);
            $query->orWhereIn('id', $result_gerentes->pluck('users_id'));
        });
        $query_verificacao_usuario->whereNotIn('id', [281,463]);
        $query_verificacao_usuario->where(function($query){
            $query->orWhereNull('folha_matricula');
            $query->orWhereNull('folha_lotacao');
        });
        $result_verificacao_cadastro_segunda = $query_verificacao_usuario->first();

        if(!empty($result_verificacao_cadastro_segunda)){
            return response()->json([
                'status' => 'error',
                'message' => 'Há Vendedor(es) Interno(s) com cadastro incompleto',
                'error' => [],
                'response' => []
            ], 422);
        }
    ...
    public function gerarIntegracaoComFolha(Request $request){
        $fields = $request->only('mes_ano', 'unidade_negocio', 'vendedor');
        
        $query_verificacao_cadastro = User::select();
        $query_verificacao_cadastro->where('tipo_usuario_id', 16);
        $query_verificacao_cadastro->whereNotIn('id', [281,463]);
        $query_verificacao_cadastro->whereNotNull('folha_matricula');
        $query_verificacao_cadastro->whereNotNull('folha_lotacao');
        $result_verificacao_cadastro = $query_verificacao_cadastro->get();

        $codigo_representante = [];
        foreach($result_verificacao_cadastro as $usuario){
            $codigo_representante[] = $usuario->codigo_representante;
        }

        $request_filtro = new Request($fields);

        $filtro = $this->filtro($request_filtro, true);

        $comissao_representante = [];
        foreach($filtro['vendedores'] as $key_unidade_negocio => $unidade_negocio){
            foreach($unidade_negocio as $comissao){
                if(empty($comissao_representante[$comissao["vendedor_codigo"]])){
                    $comissao_representante[$comissao["vendedor_codigo"]] = parserNumber($comissao["valor_total"]);
                }else{
                    $comissao_representante[$comissao["vendedor_codigo"]] += parserNumber($comissao["valor_total"]);
                }
            }
        ...
        $comissao_gerente = [];
        foreach($filtro['gerentes'] as $key_unidade_negocio => $unidade_negocio){
            if(!empty($unidade_negocio['folha_matricula_codigo']) && !empty($unidade_negocio['folha_lotacao_codigo'])){
                if(empty($comissao_gerente[intval($unidade_negocio["id_usuario"])])){
                    $comissao_gerente[intval($unidade_negocio["id_usuario"])] = [
                        'folha_matricula_codigo' => $unidade_negocio['folha_matricula_codigo'],
                        'folha_lotacao_codigo' => $unidade_negocio['folha_lotacao_codigo'],
                        'valor_total' => parserNumber($unidade_negocio['valor_total']),
                    ];
                }else{
                    $comissao_gerente[intval($unidade_negocio["id_usuario"])]['valor_total'] += parserNumber($unidade_negocio["valor_total"]);
                } 
            }
        }
        
        $comissao_representante = $this->ajusteArrayParaValores($comissao_representante);
        $comissao_gerente = $this->ajusteArrayParaValores($comissao_gerente);

        $texto = "";
        $rubrica = "0121";
        $referencia = "";
        $codigo_instituicao = [];
        $codigo_instituicao["0000005"] = "05";
        $codigo_instituicao["0000001"] = "25";

        foreach($result_verificacao_cadastro as $usuario){
            if(!empty($comissao_representante[$usuario->codigo_representante])){
                if(!empty($texto)){
                    $texto = $texto."\r\n";
                }

                $texto = $texto.$usuario->folha_matricula.";".
                        $rubrica.";".
                        parserValor($comissao_representante[$usuario->codigo_representante]).";".
                        $referencia.";".
                        $usuario->folha_lotacao.";".
                        $codigo_instituicao[$usuario->folha_lotacao];
            }
        }

        foreach($comissao_gerente as $id_usuario => $usuario){
            if(!empty($texto)){
                $texto = $texto."\r\n";
            }

            $texto = $texto.$usuario['folha_matricula_codigo'].";".
                    $rubrica.";".
                    parserValor($usuario['valor_total']).";".
                    $referencia.";".
                    $usuario['folha_lotacao_codigo'].";".
                    $codigo_instituicao[$usuario['folha_lotacao_codigo']];
        }

        $this->gerarArquivoTxt($texto, $fields['mes_ano']);

        exit;
    }

    private function gerarArquivoTxt($texto, $mes_ano){
        $arquivo = 'folha_'.$mes_ano.'.txt';
    ...
    public function modalCadastroFolha(Request $request){
        $query_gerentes = UnidadeNegocio:: select();
        $result_gerentes = $query_gerentes->get();

        $query_usuario = User::select();
        $query_usuario->where(function($query) use($result_gerentes){
            $query->orWhere('tipo_usuario_id', 16);
            $query->orWhereIn('id', $result_gerentes->pluck('users_id'));
        });
        $query_usuario->whereNotIn('id', [281,463]);
        $query_usuario->where(function($query){
            $query->orWhereNull('folha_matricula');
            $query->orWhereNull('folha_lotacao');
        });

        $result_usuario = $query_usuario->get();

        $usuarios = [];
        foreach($result_usuario as $usuario){
            $usuarios[] = [
                'id' => $usuario->id,
                'nome' => empty($usuario->codigo_representante)? $usuario->name : $usuario->codigo_representante." - ".$usuario->name,
            ];
        }

        $userControllerObj = new UserController;
        $instituicoes = $userControllerObj->getInstituicao();

        return view('programs.acompanhamento_comissao.modal.cadastro_folha')->with(['usuarios' => $usuarios, 'instituicoes' => $instituicoes]);
    }

    public function atualizarCadastroUserFolha(Request $request){
        $fields = $request->only('array_usuarios');

        foreach($fields['array_usuarios'] as $usuario){
            $userObj = User::find($usuario[0]);
            $userObj->folha_matricula = $usuario[1];
            $userObj->folha_lotacao = $usuario[2];
            $userObj->save();
        }
