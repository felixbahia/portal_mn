...
class CampanhaRegistrarRequest extends FormRequest
{
    /**
    ...
    {   
        ...
        function($attribute, $value, $fail){
            if(empty($this->request->get('produto_selecionado'))){
                return $fail('Nenhum produto foi adicionado a campanha.');
            }
        },
        function($attribute, $value, $fail){
            $verifica_duplicidade = Campanha::where('nome','ilike',trim($value))->first();
            
            if(!empty($verifica_duplicidade)){
                    ...
                    function($attribute, $value, $fail) use ($key){
                        if(!empty($this->data_inicio_campanha) && !empty($this->data_fim_campanha)){
                            ...
                            if($data_fim_apuracao_vendedor < $data_inicio_apuracao_vendedor_atual){
                                return $fail(__('validation.before', ['attribute' => 'Data Final do Período de Apuração de comissionamento23','date' => 'Data Final do período digitado']));
                            }
                        }
                    }
                ];
    ...
    public function messages(){
        $messages = [
            ...
            'nome_campanha.unique' => __('validation.unique', ['attribute' => 'Nome da Campanha']),
