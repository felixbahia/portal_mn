...
class LancamentoDebCredVendedorEditarRequest extends FormRequest
{
    /**
            ...
            'documento' =>[
                ...
                function($attribute, $value, $fail){
                    if(!empty($this->documento) && !empty($this->tipo) && !empty($this->data) && !empty($this->motivo) && !empty($this->vendedor)){
                        ...
                        $query->where('num_documento', $this->documento);
                        $query->where('tipo', $this->tipo);
                        $query->where('data_lancamento',$this->data);
                        $query->where('codigo_motivo', $this->motivo);
                        $query->where('codigo_vendedor', $this->vendedor);
    ...
    public function messages()
    {
        return [
            ...
            'documento.max' => __('validation.max', ['attribute', 'Pedido']),
            ...
            'vendedor.unique' => '',
            'documento.unique' => 'Já foram cadastradas lançamentos para este Documento',
            'data.unique' => '',
            'motivo.unique' => '',
            'tipo.unique' => '',
