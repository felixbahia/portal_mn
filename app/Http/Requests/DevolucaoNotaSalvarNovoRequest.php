...
class DevolucaoNotaSalvarNovoRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        $notaNasajon = NotaVendaNasajon::with('faturamento_nota_devolucao', 'item', 'item.produto_detalhes')
            ->where('cod_estabelecimento', str_pad($this->estabelecimento, 2, '0', STR_PAD_LEFT))
            ->where(DB::Raw('trim(leading \'0\' from numero)'), ltrim($this->nota_fiscal, 0))->first();
        ...
        if($devolucaoNotaObj->whereIn('status', [1,2])->isNotEmpty()){
        ...
        if((!in_array(Auth::user()->id, [105, 46]) && Auth::user()->tipo_usuario_id != 1) && $notaNasajon->item->pluck('produto_detalhes')->flatten()->contains('linha', 'OUTLET')){
            $outlet = true;
        }
        else{
            $outlet = false;
        ...
        $rules = [
            ...
            'produtos.*.quantidade_devolvida' => [
                ...
                function ($attribute, $value, $fail) use($itens_nao_devolvidos){
                    ...
                    if(parserNumber($value) > round($item['quantidade'], 2)){
