...
class ProdutoTecidoBaseEditarRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            'codigo_produto' => [
                'required',
                'max:60',
                Rule::unique('produto_tecidos_bases')
                    ->where('codigo_produto', $this->codigo_produto)
                    ->whereNull('deleted_at')
                    ->ignore(decrypt($this->id), 'id'),
    ...
    public function messages()
    {
        return [
            ...
            'codigo_produto_base.required' => __('validation.required', ['attribute' => 'Código Produto Base']),
            ...
            'codigo_produto_base.max' => __('validation.max', ['attribute' => 'Código Produto Base']),
            'descricao_produto.max' => __('validation.max', ['attribute' => 'Descrição Produto']),

            'codigo_produto.unique'  => 'Este Tecido já tem Código Base.',
        ];
    }
