...
class AdicionarVolumeCteEntrada extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'importacao:adicionar_volume_cte {inicio_periodo=0} {fim_periodo=0}';
     ...
     * @var string
     */
    protected $description = 'Adicionar Volume das CTE importadas.';
    ...
    {
        $AtualizacaoCronObj = AtualizacaoCron::select()->where('token', 'importacao:volume_cte_anteriores')->first();
        ...
        if($inicio_atualizacao->lte($final_atualizacao)){
            ...
            if(empty($inicio_periodo)){
                $inicio_periodo = Carbon::now()->subDays(180);
            }else{
            ...
            try{
                ...
                while($inicio_periodo->lte($fim_periodo)){
                    ...
                    $retorno->importarVolumesCTE($inicio_periodo,$data_adicional);
                    $contador ++;
